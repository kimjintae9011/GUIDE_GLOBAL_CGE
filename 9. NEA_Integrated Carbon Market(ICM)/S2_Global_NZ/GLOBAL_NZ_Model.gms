...
*$GDXIN Input_CGE/DATA_AGG-2019_GTAP11c_new.gdx
...
Parameter
S2_EMISSIONS(z,time)
BAU_EMISSIONS(z,time)
;
$CALL gdxxrw Input_CGE\S2_EMISSIONS.xlsx par=S2_EMISSIONS rng=S2_EMISSIONS!A1 Rdim=1 Cdim=1 output=Input_CGE\S2_EMISSIONS.gdx
$GDXIN Input_CGE\S2_EMISSIONS.gdx
$LOAD S2_EMISSIONS

$CALL gdxxrw Input_CGE\S2_EMISSIONS.xlsx par=BAU_EMISSIONS rng=BAU_EMISSIONS!A1 Rdim=1 Cdim=1 output=Input_CGE\BAU_EMISSIONS.gdx
$GDXIN Input_CGE\BAU_EMISSIONS.gdx
$LOAD BAU_EMISSIONS
*===============================================================================
...
PARAMETER
 ...
 REVENUE_SHARE_VARO(z)
 NEW_PERMIT_CAP_VARO(z)
;
 CTAXO(z)           = 0;
 Global_CTAXO       = 0;
 TCTAXO(z)          = 0;
 ...
 REVENUE_SHARE_VARO(z) =(BAU_EMISSIONS(z,'2019') - PERMIT_TOTALO(z)) 
        / (SUM(zj, BAU_EMISSIONS(zj,'2019') - PERMIT_TOTALO(zj)));
 NEW_PERMIT_CAP_VARO(z) = SUM(zj, S2_EMISSIONS(zj,'2019'))/10000 * REVENUE_SHARE_VARO(z);
...
* --- [4] Policy & Other Variables ---
 ...
 Global_CTAX(time)  Global Carbon tax
 TCTAX(z,time)           Government revenue from Carbon tax
 REVENUE_SHARE_VAR(z,time)   
 NEW_PERMIT_CAP_VAR(z,time)  
...
*==============================================================================
 ...
 EQ34(z,t)..
    SROW(z,t) =e= YROW(z,t) - e(z,t) * SUM[(i,zj)$EXO(i,z,zj), EX(i,z,zj,t) * PWX(i,z,zj,t)]
                  ...
                  - (NEW_PERMIT_CAP_VAR(z,t)- PERMIT_TOTAL(z,t)) * GLOBAL_CTAX(t);
...
*==============================================================================
 ...
 EQ_Global_Cap(t).. 
    SUM(z, PERMIT_TOTAL(z,t)) =e= sum(z,S2_EMISSIONS(z,t))/10000;

 EQ_Rev_Share(z,t)..
    REVENUE_SHARE_VAR(z,t) =e= (BAU_EMISSIONS(z,t) - PERMIT_TOTAL(z,t)) 
        / (SUM(zj, BAU_EMISSIONS(zj,t) - PERMIT_TOTAL(zj,t)));

 EQ_New_Cap(z,t)..
    NEW_PERMIT_CAP_VAR(z,t) =e= SUM(zj, S2_EMISSIONS(zj,t))/10000 * REVENUE_SHARE_VAR(z,t);

*==============================================================================
* 8 Carbon Tax Revenue Recycling
*==============================================================================
 EQ109(z,t)..
    REBATE(z,t) =e= TCTAX(z,t)+ (NEW_PERMIT_CAP_VAR(z,t) - PERMIT_TOTAL(z,t)) * GLOBAL_CTAX(t);
...
*==============================================================================
$INCLUDE Global_NZ_SOLVE.gms
$INCLUDE Global_NZ_RESULTS.gms
