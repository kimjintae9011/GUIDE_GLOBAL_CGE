...
$GDXIN Input_w-1/DATA_AGG-2019_230906_POWER3_Joint.gdx
...
execute_unload 'CO2FACTOR_w-1_GTAP10',
 CO2FACTOR ;
...
PARAMETER
 ...
 valGDP_BP(z,sim)       Region z GDP at basic prices
 valGDP_BP_REAL(z,sim)  Real GDP at basic prices
 ...
 valTCTAX(z,sim)        Government revenue from Carbon tax
 ...
 valKLE(j,z,sim)        KLE of industry j in region z
...
*execute_unload 'CAL_Pivot too small' ;
*$exit
...
*EQ9_2(ene,j2,z)..  DE(ene,j2,z) =e= [beta_ENER(ene,j2,z)*PCE(j2,z)/P4(ene,j2,z)]
...
* EQ9_2(ene,j2,z)..  DE(ene,j2,z) =e= [beta_ENER(ene,j2,z)*PCE(j2,z)/((1+CTAX(Z)*CO2FACTOR(ene,j2,z))*P4(ene,j2,z))]
*                                   **sigma_ENER(j2,z)*B_ENER(j2,z)**(sigma_ENER(j2,z)-1)
*                                   *CE(j2,z);

 
* EQ9_3(j2,z)..      CE(j2,z) =e= B_ENER(j2,z)*SUM[ene$DEO(ene,j2,z),beta_ENER(ene,j2,z)
...
*eqTe(j)..       Te(j)   =e= sum(en,taue(en)*pq(en)*CO2coef(en)*X(en,j)); 
 ...
 EQ21(z)..       TPRCTS(z) =e= TICT(z)+TIMT(z)+TIXT(z)+TCTAX(z);
...
*                  SUM[j,{(PVA(j,z)*VA(j,z)+TIP(j,z))}]/sum[j,{PVAO(j,z)*VAO(j,z)+TIPO(j,z)}] ;         
...
*                  SUM[(j,z),{(PVA(j,z)*VA(j,z)+TIP(j,z))}]/sum[(j,z),{PVAO(j,z)*VAO(j,z)+TIPO(j,z)}] ;         
...
*==============================================================================
 ...
 EQ84(z)..       GDP_FD(z) =e=
                ...
                -SUM[(i,zj)$IMO(i,zj,z),e(z)*IM(i,zj,z)
                    *(PWM(i,zj,z)+SUM[ij,PWMG(ij)*tmrg(ij,i,zj,z)])]-TCTAX(z);
...
$ontext
Parameter numeraire;
* In the default specification, the numeraire is equal to the
* reference region's GDP deflator:
* numeraire = SUM[zr,PIXGDPO(zr)];
* In the preceding expression, the sum is taken over the set zr,
* which contains a single element.

* Next, assign the numeraire any desired value.
* For example
numeraire = 1.2*SUM[zr,PIXGDPO(zr)];
* Then apply the closure rules
PIXGDP.FX(zr) = numeraire;
e.FX(z) = numeraire*eO(z);
...
execute_unload 'Output_w-1/results_PEP-w-1_v4.0_GTAP10_231208_CTAX.gdx',
