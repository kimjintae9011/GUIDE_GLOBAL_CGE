...
* CES - composite ENER
 sigma_ENER_elec(j2,z)         = 2.0;
 sigma_ENER_nelec(j2,z)        = 2.0;
 sigma_ENER_coalgas(j2,z)      = 1.5;
...
PARAMETER
 ...
 shift_val(j, z)        Value transfer for process transition
 TSFO(j, z)             Initial year TSF input for backstop (Value)
 PTSFO(j, z)            Initial year TSF price for backstop
 Green_Markup(j, z)     Initial efficiency penalty for backstop (Price markup)
;

* 모든 백스톱 산업에 대한 초기값 세팅
seed(BS) = 0.001;
PTSFO(BS, Z_GRN) = 1.0;       
...
*--------------------------------------------------------------------------
* [1] Basic Data Allocation for New Technologies
*--------------------------------------------------------------------------
  LOOP(MAP_BS(j, jj, i),
...
* 중간재 투입은 전체 상품(ii)에 대해 돌려야 하므로 i와 겹치지 않게 ii 사용
    DIO(ii, j, Z_GRN)         = DIO(ii, jj, Z_GRN) * seed(j);

* 산출물(XSO)과 공급(DSO)은 상품(i) 도메인 규칙 준수
...
*--------------------------------------------------------------------------
* [2] Characterization: Rebalancing Intermediate Input Structure
*--------------------------------------------------------------------------
* (1) 그린 화학: 석유/가스 -> 전력/바이오
  shift_val('32_BSCHEMICAL', Z_GRN) 
      = DIO('10_PETROLCOAL', '32_BSCHEMICAL', Z_GRN) * 0.9
      + DIO('04_GAS', '32_BSCHEMICAL', Z_GRN) * 0.8
      + DIO('03_OIL', '32_BSCHEMICAL', Z_GRN) * 0.8;
      
  DIO('10_PETROLCOAL', '32_BSCHEMICAL', Z_GRN) = DIO('10_PETROLCOAL', '32_BSCHEMICAL', Z_GRN) * 0.1;
  DIO('04_GAS', '32_BSCHEMICAL', Z_GRN)        = DIO('04_GAS', '32_BSCHEMICAL', Z_GRN) * 0.2;
  DIO('03_OIL', '32_BSCHEMICAL', Z_GRN)        = DIO('03_OIL', '32_BSCHEMICAL', Z_GRN) * 0.2;
  
  DIO('18_ELEC', '32_BSCHEMICAL', Z_GRN)       = DIO('18_ELEC', '32_BSCHEMICAL', Z_GRN) + shift_val('32_BSCHEMICAL', Z_GRN) * 0.8;
  DIO('01_AGRICULT', '32_BSCHEMICAL', Z_GRN)   = DIO('01_AGRICULT', '32_BSCHEMICAL', Z_GRN) + shift_val('32_BSCHEMICAL', Z_GRN) * 0.2;

* (2) 그린 시멘트: 석탄/석유 -> 전력/가스
  shift_val('33_BSNONMET', Z_GRN) 
      = DIO('02_COAL', '33_BSNONMET', Z_GRN) * 0.9
      + DIO('10_PETROLCOAL', '33_BSNONMET', Z_GRN) * 0.9;
      
  DIO('02_COAL', '33_BSNONMET', Z_GRN)         = DIO('02_COAL', '33_BSNONMET', Z_GRN) * 0.1;
  DIO('10_PETROLCOAL', '33_BSNONMET', Z_GRN)   = DIO('10_PETROLCOAL', '33_BSNONMET', Z_GRN) * 0.1;
  
  DIO('18_ELEC', '33_BSNONMET', Z_GRN)         = DIO('18_ELEC', '33_BSNONMET', Z_GRN) + shift_val('33_BSNONMET', Z_GRN) * 0.8;
  DIO('04_GAS', '33_BSNONMET', Z_GRN)          = DIO('04_GAS', '33_BSNONMET', Z_GRN) + shift_val('33_BSNONMET', Z_GRN) * 0.2;

* (3) 그린 스틸: 광물/석탄/석유 -> 전력/가스
  shift_val('34_BSIRONSTL', Z_GRN) 
      = DIO('05_MINING', '34_BSIRONSTL', Z_GRN) * 0.7 
      + DIO('02_COAL', '34_BSIRONSTL', Z_GRN) * 0.9 
      + DIO('10_PETROLCOAL', '34_BSIRONSTL', Z_GRN) * 0.9;

  DIO('05_MINING', '34_BSIRONSTL', Z_GRN)      = DIO('05_MINING', '34_BSIRONSTL', Z_GRN) * 0.3;
  DIO('02_COAL', '34_BSIRONSTL', Z_GRN)        = DIO('02_COAL', '34_BSIRONSTL', Z_GRN) * 0.1;
  DIO('10_PETROLCOAL', '34_BSIRONSTL', Z_GRN)  = DIO('10_PETROLCOAL', '34_BSIRONSTL', Z_GRN) * 0.1;

  DIO('18_ELEC', '34_BSIRONSTL', Z_GRN)        = DIO('18_ELEC', '34_BSIRONSTL', Z_GRN) + shift_val('34_BSIRONSTL', Z_GRN) * 0.8;
  DIO('04_GAS', '34_BSIRONSTL', Z_GRN)         = DIO('04_GAS', '34_BSIRONSTL', Z_GRN)  + shift_val('34_BSIRONSTL', Z_GRN) * 0.2;

* (4) 전기차/수소차 육상수송: 석유 -> 전력
  shift_val('35_BS_LTRP', Z_GRN) 
      = DIO('10_PETROLCOAL', '35_BS_LTRP', Z_GRN) * 0.95;
      
  DIO('10_PETROLCOAL', '35_BS_LTRP', Z_GRN)    = DIO('10_PETROLCOAL', '35_BS_LTRP', Z_GRN) * 0.05;
  DIO('18_ELEC', '35_BS_LTRP', Z_GRN)          = DIO('18_ELEC', '35_BS_LTRP', Z_GRN) + shift_val('35_BS_LTRP', Z_GRN);

* (5) 친환경 해운수송: 석유 -> 화학제품(메탄올/암모니아)
  shift_val('36_BS_WTRP', Z_GRN) 
      = DIO('10_PETROLCOAL', '36_BS_WTRP', Z_GRN) * 0.95;
      
  DIO('10_PETROLCOAL', '36_BS_WTRP', Z_GRN)    = DIO('10_PETROLCOAL', '36_BS_WTRP', Z_GRN) * 0.05;
  DIO('11_CHEMICAL', '36_BS_WTRP', Z_GRN)      = DIO('11_CHEMICAL', '36_BS_WTRP', Z_GRN) + shift_val('36_BS_WTRP', Z_GRN);

* (6) SAF 항공수송: 석유 -> 화학제품(SAF)
  shift_val('37_BS_ATRP', Z_GRN) 
      = DIO('10_PETROLCOAL', '37_BS_ATRP', Z_GRN) * 0.95;
      
  DIO('10_PETROLCOAL', '37_BS_ATRP', Z_GRN)    = DIO('10_PETROLCOAL', '37_BS_ATRP', Z_GRN) * 0.05;
  DIO('11_CHEMICAL', '37_BS_ATRP', Z_GRN)      = DIO('11_CHEMICAL', '37_BS_ATRP', Z_GRN) + shift_val('37_BS_ATRP', Z_GRN);

*--------------------------------------------------------------------------
* [3] Technology Specific Factor (TSF) Allocation for All Backstops
*--------------------------------------------------------------------------
  LOOP(BS,
...
*--------------------------------------------------------------------------
* [4] Accounting Identity Preservation: Subtract from Conventional Sectors
*--------------------------------------------------------------------------
  LOOP(MAP_BS(j, jj, i),
    ...
    DIO(ii, jj, Z_GRN)             = DIO(ii, jj, Z_GRN) - DIO(ii, j, Z_GRN);
  );

);

* Calculate TSF intensity and assign initial values to time series for all backstops
LOOP(BS,
...
$Ontext
*==============================================================================
* Data Split for 32nd Industry: 32_BSIRONSTL (Green Steel)
*==============================================================================
PARAMETER
 seed(Z_GRN)          Initial share of the new technology
 shift_val            Value transfer for green steel process transition
 TSFO(Z_GRN)          Initial year TSF input for green steel (Value)
 PTSFO(Z_GRN)         Initial year TSF price for green steel
 Green_Markup(Z_GRN)  Initial efficiency penalty for green steel (Price markup)
;

seed(Z_GRN)              = 0.001;
PTSFO(Z_GRN)            = 1.0;       
Green_Markup(Z_GRN) = 1.0;

* Loop over Z_GRN (e.g., Korea) to prevent index errors (Error 149)
LOOP(Z_GRN,
*--------------------------------------------------------------------------
* [1] Basic Data Allocation for New Technology (32_BSIRONSTL)
*--------------------------------------------------------------------------
  LDO('32_BSIRONSTL', Z_GRN)               = LDO('13_IRONSTL', Z_GRN) * seed(Z_GRN);
  RKDO(k, '32_BSIRONSTL', Z_GRN)           = RKDO(k, '13_IRONSTL', Z_GRN) * seed(Z_GRN);
  TIPO('32_BSIRONSTL', Z_GRN)              = TIPO('13_IRONSTL', Z_GRN) * seed(Z_GRN);
  TIWO('32_BSIRONSTL', Z_GRN)              = TIWO('13_IRONSTL', Z_GRN) * seed(Z_GRN);
  TIKO(k, '32_BSIRONSTL', Z_GRN)           = TIKO(k, '13_IRONSTL', Z_GRN) * seed(Z_GRN);
  DIO(i, '32_BSIRONSTL', Z_GRN)            = DIO(i, '13_IRONSTL', Z_GRN) * seed(Z_GRN);

  XSO('32_BSIRONSTL', '13_IRONSTL', Z_GRN) = (XSO('13_IRONSTL', '13_IRONSTL', Z_GRN) * seed(Z_GRN)) / Green_Markup(Z_GRN);
  XSTO('32_BSIRONSTL', Z_GRN)              = (XSTO('13_IRONSTL', Z_GRN) * seed(Z_GRN)) / Green_Markup(Z_GRN);
  DSO('32_BSIRONSTL', '13_IRONSTL', Z_GRN) = (DSO('13_IRONSTL', '13_IRONSTL', Z_GRN) * seed(Z_GRN)) / Green_Markup(Z_GRN);

*--------------------------------------------------------------------------
* [2] Green Steel Characterization: Rebalancing Intermediate Input Structure
*--------------------------------------------------------------------------
  shift_val 
      = DIO('05_MINING', '32_BSIRONSTL', Z_GRN) * 0.7 
      + DIO('02_COAL', '32_BSIRONSTL', Z_GRN) * 0.9 
      + DIO('10_PETROLCOAL', '32_BSIRONSTL', Z_GRN) * 0.9;

  DIO('05_MINING', '32_BSIRONSTL', Z_GRN)     = DIO('05_MINING', '32_BSIRONSTL', Z_GRN) * 0.3;
  DIO('02_COAL', '32_BSIRONSTL', Z_GRN)       = DIO('02_COAL', '32_BSIRONSTL', Z_GRN) * 0.1;
  DIO('10_PETROLCOAL', '32_BSIRONSTL', Z_GRN) = DIO('10_PETROLCOAL', '32_BSIRONSTL', Z_GRN) * 0.1;

  DIO('18_ELEC', '32_BSIRONSTL', Z_GRN)       = DIO('18_ELEC', '32_BSIRONSTL', Z_GRN) + shift_val * 0.8;
  DIO('04_GAS', '32_BSIRONSTL', Z_GRN)        = DIO('04_GAS', '32_BSIRONSTL', Z_GRN)  + shift_val * 0.2;

  TSFO(Z_GRN) 
      = (RKDO('cap', '32_BSIRONSTL', Z_GRN) * 0.9) / PTSFO(Z_GRN);

  RKDO('cap', '32_BSIRONSTL', Z_GRN) 
      = RKDO('cap', '32_BSIRONSTL', Z_GRN) - (PTSFO(Z_GRN) * TSFO(Z_GRN));

*--------------------------------------------------------------------------
* [3] Accounting Identity Preservation: Subtract from Conventional Steel
*--------------------------------------------------------------------------
  LDO('13_IRONSTL', Z_GRN)                 = LDO('13_IRONSTL', Z_GRN) * (1 - seed(Z_GRN));
  RKDO(k, '13_IRONSTL', Z_GRN)             = RKDO(k, '13_IRONSTL', Z_GRN) * (1 - seed(Z_GRN));
  XSO('13_IRONSTL', '13_IRONSTL', Z_GRN)   = XSO('13_IRONSTL', '13_IRONSTL', Z_GRN) * (1 - seed(Z_GRN));
  XSTO('13_IRONSTL', Z_GRN)                = XSTO('13_IRONSTL', Z_GRN) * (1 - seed(Z_GRN));
  TIPO('13_IRONSTL', Z_GRN)                = TIPO('13_IRONSTL', Z_GRN) * (1 - seed(Z_GRN));
  TIWO('13_IRONSTL', Z_GRN)                = TIWO('13_IRONSTL', Z_GRN) * (1 - seed(Z_GRN));
  TIKO(k, '13_IRONSTL', Z_GRN)             = TIKO(k, '13_IRONSTL', Z_GRN) * (1 - seed(Z_GRN));
  DSO('13_IRONSTL', '13_IRONSTL', Z_GRN)   = DSO('13_IRONSTL', '13_IRONSTL', Z_GRN) * (1 - seed(Z_GRN));

  DIO(i, '13_IRONSTL', Z_GRN)              = DIO(i, '13_IRONSTL', Z_GRN) - DIO(i, '32_BSIRONSTL', Z_GRN);
);

 v_tsf(j, Z_GRN)    = TSFO(Z_GRN) / XSTO(j, Z_GRN);
 
 TSFS(Z_GRN, time)  = TSFO(Z_GRN);
$Offtext
*==============================================================================
...
*==============================================================================
 ...
 YHO(z) = YHLO(z) + YHKO(z) + SUM((BS, Z_GRN)$sameas(z, Z_GRN), PTSFO(BS,Z_GRN) * TSFO(BS,Z_GRN));
 YDHO(z)          = YHO(z) - TDHO(z);
 CTHO(z)          = SUM[i, CO(i,z)];
 SHO(z)           = YDHO(z) - CTHO(z);
...
*==============================================================================
 ...
 PPO(j,z)$XSTO(j,z) 
    = ( TIWO(j,z) + WO(z) * LDO(j,z)
      ...
      + SUM((BS,Z_GRN)$sameas(z, Z_GRN), PTSFO(BS,Z_GRN) * TSFO(BS,Z_GRN))
...
* Backstop Sectors
* IRONSTLQO(Z_GRN) = SUM(IRONSTL, XSO(IRONSTL,'13_IRONSTL',Z_GRN));
* rho_X5(Z_GRN)    = (1 - sigma_X5(Z_GRN)) / sigma_X5(Z_GRN);

* beta_X5(IRONSTL,Z_GRN)
*    = [ PO2(IRONSTL,'13_IRONSTL',Z_GRN) * XSO(IRONSTL,'13_IRONSTL',Z_GRN)**(1 + rho_X5(Z_GRN)) ] /
*      SUM[ironstll$XSO(ironstll,'13_IRONSTL',Z_GRN), PO2(ironstll,'13_IRONSTL',Z_GRN) * XSO(ironstll,'13_IRONSTL',Z_GRN)**(1 + rho_X5(Z_GRN)) ];

* B_X5(Z_GRN)
*    = IRONSTLQO(Z_GRN) / 
*      SUM[ironstll$XSO(ironstll,'13_IRONSTL',Z_GRN), beta_X5(ironstll,Z_GRN) * XSO(ironstll,'13_IRONSTL',Z_GRN)**(-rho_X5(Z_GRN))]**(-1 / rho_X5(Z_GRN));

* beta_X5_t(IRONSTL,Z_GRN,time) = beta_X5(IRONSTL,Z_GRN);
 
PBSO(BS, Z_GRN) = 1.0;

LOOP(MAP_BS(bs, conv, I_BS),

* 산출량 총합 (기존 기술 생산량 + 백스톱 기술 생산량)
...
* VATO(Z_GRN)      = VAO('32_BSIRONSTL', Z_GRN) + TSFO(Z_GRN);
* PVATO(Z_GRN)     = (PVAO('32_BSIRONSTL', Z_GRN) * VAO('32_BSIRONSTL', Z_GRN) 
*                  + PTSFO(Z_GRN) * TSFO(Z_GRN)) / VATO(Z_GRN);
* rho_VAT(Z_GRN)   = (1 - sigma_VAT(Z_GRN)) / sigma_VAT(Z_GRN);

* beta_VAT(Z_GRN)  
*    = (VAO('32_BSIRONSTL', Z_GRN)**(1 + rho_VAT(Z_GRN))) / 
*      (VAO('32_BSIRONSTL', Z_GRN)**(1 + rho_VAT(Z_GRN)) + TSFO(Z_GRN)**(1 + rho_VAT(Z_GRN)));

* B_VAT(Z_GRN)     
*    = VATO(Z_GRN) / 
*      ( beta_VAT(Z_GRN) * VAO('32_BSIRONSTL', Z_GRN)**(-rho_VAT(Z_GRN)) 
*      + (1 - beta_VAT(Z_GRN)) * TSFO(Z_GRN)**(-rho_VAT(Z_GRN)) )**(-1 / rho_VAT(Z_GRN));
...
*==============================================================================
 ...
 PVATO(BS, Z_GRN)     = ( PVAO(BS, Z_GRN) * VAO(BS, Z_GRN) 
                        + PTSFO(BS, Z_GRN) * TSFO(BS, Z_GRN) ) / VATO(BS, Z_GRN);
                        
 rho_VAT(BS, Z_GRN)   = (1 - sigma_VAT(BS, Z_GRN)) / sigma_VAT(BS, Z_GRN);

 beta_VAT(BS, Z_GRN)  
    = (VAO(BS, Z_GRN)**(1 + rho_VAT(BS, Z_GRN))) / 
      (VAO(BS, Z_GRN)**(1 + rho_VAT(BS, Z_GRN)) + TSFO(BS, Z_GRN)**(1 + rho_VAT(BS, Z_GRN)));
...
*==============================================================================
 ...
 PKLEO(j,z)$XSTO(j,z) = [PCEO(j,z) * CEO(j,z) + PVAO(j,z) * VAO(j,z)] / KLEO(j,z);
...
;
 ...
 elas_elec(j2,z)       = -1 * (1 - DEO('18_ELEC',j2,z) / SUM(ene, DEO(ene,j2,z))) * sigma_ENER_elec(j2,z);
 elas_elec(j3,z)       = eps;
 elas_gas(j2,z)        = -1 * (1 - DEO('04_GAS',j2,z) / (DEO('02_COAL',j2,z) + DEO('04_GAS',j2,z))) * sigma_ENER_coalgas(j2,z);
 elas_gas(j3,z)        = eps;
 elas_coal(j2,z)       = -1 * (1 - DEO('02_COAL',j2,z) / (DEO('02_COAL',j2,z) + DEO('04_GAS',j2,z))) * sigma_ENER_coalgas(j2,z);
 elas_coal(j3,z)       = eps;
 elas_oil(j2,z)        = -1 * (1 - DEO('03_OIL',j2,z) / (DEO('03_OIL',j2,z) + DEO('10_PETROLCOAL',j2,z))) * sigma_ENER_oilprod(j2,z);
 elas_oil(j3,z)        = eps;

 elas_petrolcoal(j2,z) = -1 * (1 - DEO('10_PETROLCOAL',j2,z) / (DEO('03_OIL',j2,z) + DEO('10_PETROLCOAL',j2,z))) * sigma_ENER_oilprod(j2,z);
 elas_petrolcoal(j3,z) = eps;
 
 elas_Hou_ENER(ene,z)  = CMINO(ene,z) / CO(ene,z) * (1 - gamma_LES(ene,z)) - 1; 
 
*==============================================================================
* Sigma_ENE_elec Normalization
*==============================================================================
PARAMETER W_Origin(ene,j,z) "Original Weights";
W_Origin(ene,j,z)$(SUM(jj, DIO(ene,jj,z))) = DIO(ene,j,z) / SUM(jj, DIO(ene,jj,z));
W_Origin(ene,j,z)$(not SUM(jj, DIO(ene,jj,z))) = 0;

PARAMETER V_log(ene,j2,z)   "Log transformed weights (ln(W_orig))";
PARAMETER V_min(ene), V_max(ene);
PARAMETER N_norm(ene,j2,z)  "Normalized values (0 to 1)";
PARAMETER W_final(ene,j2,z) "Final Scaled Weights (0.1 to 2.0)";
SCALAR A /0.1/, B /2.0/;

* 1. Log transformation
V_log(ene,j2,z) = LOG(W_Origin(ene,j2,z) + 1E-8);

* 2. Minimum and Maximum
V_min(ene) = SMIN((j2,z), V_log(ene,j2,z));
V_max(ene) = SMAX((j2,z), V_log(ene,j2,z));

* 3. 0-1 Normalization
N_norm(ene,j2,z) = 0.5;

N_norm(ene,j2,z)$(V_max(ene) - V_min(ene)) = 
    (V_log(ene,j2,z) - V_min(ene)) / (V_max(ene) - V_min(ene));

* 4. Final scaling to target range
W_final(ene,j2,z) = A + (B - A) * N_norm(ene,j2,z);
...
* --- [1] Volume Variables (Real Quantities) ---
 ...
 IRONSTLQ(Z_GRN,time)    Total ironstl output in region z
 Q_DAC(z,time)           100ktCO2 
 TSFD(j,Z_GRN,time)      Demand for TSF
 VAT(Z_GRN,time)         Quantity of Value-Added and TSF composite
...
* --- [2] Price Variables ---
 ...
 PIRONSTL(Z_GRN,time)    Basic price of composite ironstl commodity in region z
 ...
 PVAT(Z_GRN,time)        Price of VAT composite
 PTSF(Z_GRN,time)        Price of TSF
...
* --- [4] Policy & Other Variables ---
 ...
 PERMIT_NEA_TOTAL(time)  Total permits in NEA
...
* --- Nested Trade & Output ---
 ...
 EQA_3(Z_GRN,time)       Top Nest - IRONSTAL
 EQA_3_OTH(Z_OTH, time)  Top Nest - OTH IRONSTAL
 ...
 EQA_6(Z_GRN,time)       Second Nest - CES function - Aggregated-IRONSTL
 EQA_7(IRONSTL,i,Z_GRN,time) Second Nest - CES function - Aggregated-IRONSTL
...
* --- Climate Policy (CTAX, Permits, DAC) ---
 ...
 EQ108_1(time)           Total permits NEA
...
* --- Green Steel (VAT Nest) ---
 EQ_TSFMKT(Z_GRN,time)   TSF Market clearing condition
 EQ_TSFD(Z_GRN,time)     Demand function for TSF (Leontief)
 EQ_VAT_REQ(Z_GRN, time) VAT composite requirement
 EQ_PVAT(Z_GRN, time)    Price of VAT composite
 EQ_VA_GRN(Z_GRN, time)  Value added for Green Steel
 EQ_PP_GRN(Z_GRN, time)  Producer price for Green Steel
 EQ_PVA_GRN(Z_GRN, time) PVA for Green Steel
;
...
* 5.3.1 Production
*==============================================================================
 EQ1(j3,z,t)$[not sameas(j3,'32_BSIRONSTL')]..
 ...
 EQ4_1(j2,z,t)$[VAO(j2,z) and CEO(j2,z)]..
    VA(j2,z,t) =e= {[(1 - beta_KLE2(j2,z)) / (beta_KLE2(j2,z))]
 ...
 EQ9(nene,j,z,t)..
    DI(nene,j,z,t) =e= aij(nene,j,z) * CI(j,z,t);
...
*==============================================================================
 EQ10(z,t)..
    YH(z,t) =e= YHL(z,t) + YHK(z,t) + REBATE(z,t) * recycle_hou(z,t) + SUM(Z_GRN$sameas(z, Z_GRN), PTSF(Z_GRN,t) * TSFD('32_BSIRONSTL', Z_GRN, t));
...
*==============================================================================
 EQ16(z,t)..
    YG(z,t) =e= TDH(z,t) + TPRODN(z,t) + TPRCTS(z,t) + REBATE(z,t) * recycle_gov(z,t) - SUM(i1, PC(i1,z,t) * Q_DAC(z,t) * DAC_Tech_Coeff(i1));
...
*==============================================================================
 EQ33(z,t)..
    YROW(z,t) =e= e(z,t) * SUM{(i,zj)$IMO(i,zj,z), IM(i,zj,z,t) * [PWM(i,zj,z,t) + SUM(ij, PWMG(ij,t) * tmrg(ij,i,zj,z))]};
...
*==============================================================================
 ...
 EQA_2(z,t)..
    XS_I('18_ELEC',z,t) =e= XS('18_TnD','18_ELEC',z,t) + POWERQ(z,t);

 EQA_3(Z_GRN,t)..
    XS_I('13_IRONSTL',Z_GRN,t) =e= IRONSTLQ(Z_GRN,t);

 EQA_3_OTH(Z_OTH, t)..
    XS_I('13_IRONSTL', Z_OTH, t) =e= XS('13_IRONSTL', '13_IRONSTL', Z_OTH, t);

 EQA_4(z,t)..
    POWERQ(z,t) =e= B_X4(z) * SUM[POWER, beta_X4(power,z) * XS(POWER,'18_ELEC',z,t)**(-rho_X4(z))]**(-1 / rho_X4(z));

 EQA_5(power,'18_ELEC',z,t)..
    XS(POWER,'18_ELEC',z,t) =e= POWERQ(z,t) / B_X4(z)**(1 - sigma_X4(z)) *
                                [beta_X4(power,z) * PPOWER(z,t) / P2(POWER,'18_ELEC',z,t)]**sigma_X4(z);

 EQA_6(Z_GRN,t)..
    IRONSTLQ(Z_GRN,t) =e= B_X5(Z_GRN) * SUM[IRONSTL, beta_X5_t(IRONSTL,Z_GRN,t) * XS(IRONSTL,'13_IRONSTL',Z_GRN,t)**(-rho_X5(Z_GRN))]**(-1 / rho_X5(Z_GRN));

 EQA_7(IRONSTL,'13_IRONSTL',Z_GRN,t)..
    XS(IRONSTL,'13_IRONSTL',Z_GRN,t) =e= IRONSTLQ(Z_GRN,t) / B_X5(Z_GRN)**(1 - sigma_X5(Z_GRN)) *
                                         [beta_X5_t(IRONSTL,Z_GRN,t) * PIRONSTL(Z_GRN,t) / P2(IRONSTL,'13_IRONSTL',Z_GRN,t)]**sigma_X5(Z_GRN);
 ...
 EQB_2(j,i2,z,t)${DSO(j,i2,z) and [DSO(j,i2,z) ne DSO_I(i2,z)]}..
    XS(j,i2,z,t) =e= XS_I(i2,z,t) / B_X3(i2,z)**(1 - sigma_X3(i2,z)) *
                     [beta_X3(j,i2,z) * P(i2,z,t) / P2(j,i2,z,t)]**sigma_X3(i2,z);
...
*==============================================================================
 ...
 EQ49_1(j3,z,t)$[not sameas(j3,'32_BSIRONSTL')]..    
 ...
 EQ52(j,z,t)$[not sameas(j,'32_BSIRONSTL')]..
...
*==============================================================================
 EQ71(i1,z,t)..
    Q(i1,z,t) =e= C(i1,z,t) + CG(i1,z,t) + INV(i1,z,t) + DIT(i1,z,t) + (Q_DAC(z,t) * DAC_Tech_Coeff(i1));
 ...
 EQ79(i,t)..
    SUM[z$MRGNO(i,z), MRGN(i,z,t)] =e= SUM[(z,zj,ij)$IMO(ij,zj,z), tmrg(i,ij,zj,z) * IM(ij,zj,z,t)];
...
*==============================================================================
 EQ81(z,t)..
    GDP_BP(z,t) =e= SUM[j, PVA(j,z,t) * VA(j,z,t)] + TIPT(z,t)
                  + SUM(Z_GRN$sameas(z, Z_GRN), PTSF(Z_GRN,t) * TSFD('32_BSIRONSTL', Z_GRN, t));
 ...
 EQ84(z,t)..
    GDP_FD(z,t) =e= SUM[i, PC(i,z,t) * (C(i,z,t) + CG(i,z,t) + INV(i,z,t))]
                  ...
                  + SUM(i1, PC(i1,z,t) * Q_DAC(z,t) * DAC_Tech_Coeff(i1));
...
*==============================================================================
 ...
 EQ108(z,t)..
    PERMIT_TOTAL(z,t) =e= SUM(j, PERMIT(j,z,t)) - Q_DAC(z,t) * switchDAC(z,t);
 
 EQ108_1(t)..
    PERMIT_NEA_TOTAL(t) =e= SUM(NEA, PERMIT_TOTAL(NEA,t));
...
*==============================================================================
 EQ118(z,t)..
    Q_DAC(z,t) =e= DAC_MAX * DAC_growth_rate(z,t) * switchDAC(z,t);
...
*==============================================================================
 EQ_PP_GRN(Z_GRN, t)..
    PP('32_BSIRONSTL', Z_GRN, t) * XST('32_BSIRONSTL', Z_GRN,t) =e= PVAT(Z_GRN, t) * VAT(Z_GRN,t)
                                                                  + PCI('32_BSIRONSTL', Z_GRN,t) * CI('32_BSIRONSTL', Z_GRN,t)
                                                                  + PCE('32_BSIRONSTL', Z_GRN,t) * CE('32_BSIRONSTL', Z_GRN,t);

 EQ_PVA_GRN(Z_GRN, t)..
    PVA('32_BSIRONSTL', Z_GRN, t) =e= (1 / B_VA('32_BSIRONSTL', Z_GRN)) * ( 
                                      (beta_VA('32_BSIRONSTL', Z_GRN)**sigma_VA('32_BSIRONSTL', Z_GRN)) * WC('32_BSIRONSTL', Z_GRN, t)**(1 - sigma_VA('32_BSIRONSTL', Z_GRN))
                                    + ((1 - beta_VA('32_BSIRONSTL', Z_GRN))**sigma_VA('32_BSIRONSTL', Z_GRN)) * RC('32_BSIRONSTL', Z_GRN, t)**(1 - sigma_VA('32_BSIRONSTL', Z_GRN)) 
                                    )**(1 / (1 - sigma_VA('32_BSIRONSTL', Z_GRN)));

 EQ_VA_GRN(Z_GRN, t)..
    VA('32_BSIRONSTL', Z_GRN, t) =e= VAT(Z_GRN, t) 
                                   * (beta_VAT(Z_GRN) * PVAT(Z_GRN, t) / PVA('32_BSIRONSTL', Z_GRN, t))**sigma_VAT(Z_GRN) 
                                   * (B_VAT_t(Z_GRN,t))**(sigma_VAT(Z_GRN) - 1);

 EQ_VAT_REQ(Z_GRN, t)..
    VAT(Z_GRN, t) =e= v_vat(Z_GRN) * XST('32_BSIRONSTL', Z_GRN, t);

 EQ_PVAT(Z_GRN, t)..
    PVAT(Z_GRN, t) =e= (1 / B_VAT_t(Z_GRN,t)) * (
                       (beta_VAT(Z_GRN)**sigma_VAT(Z_GRN)) * PVA('32_BSIRONSTL', Z_GRN, t)**(1 - sigma_VAT(Z_GRN)) 
                     + ((1 - beta_VAT(Z_GRN))**sigma_VAT(Z_GRN)) * PTSF(Z_GRN, t)**(1 - sigma_VAT(Z_GRN)) 
                     )**(1 / (1 - sigma_VAT(Z_GRN)));

 EQ_TSFD(Z_GRN, t)..
    TSFD('32_BSIRONSTL', Z_GRN, t) =e= VAT(Z_GRN, t) 
                                     * ((1 - beta_VAT(Z_GRN)) * PVAT(Z_GRN, t) / PTSF(Z_GRN, t))**sigma_VAT(Z_GRN) 
                                     * (B_VAT_t(Z_GRN,t))**(sigma_VAT(Z_GRN) - 1);

 EQ_TSFMKT(Z_GRN,t)..
    TSFS(Z_GRN,t) =e= TSFD('32_BSIRONSTL', Z_GRN, t);
...
*$INCLUDE Homogenity_SOLVE_GTAP11c_new.gms
*$INCLUDE Ownprice_SOLVE_GTAP11c_new.gms
*$INCLUDE Ownprice_RESULTS_GTAP11c_new.gms
