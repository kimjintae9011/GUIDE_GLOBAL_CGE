...
*$CALL har2gdx basedata2019_230110.har basedata2019.gdx
*$CALL har2gdx basedata2019_230111.har basedata2019.gdx
$CALL har2gdx basedata2019_230111_MNG.har basedata2019.gdx
...
* 20_TRANSPORT    Land transport service(road rail) Water transport service Air transport service
...
/
 ...
 14_MACHINE      Fabricated metal products Electronic and electrical equipment Machinery and equipment
 15_TRANSEQ      Motor vehicles Other transport equipment
 16_OTHERIND     Other manufactured products
 17_ELEC         Electricity
 18_GAS          Gas
 19_CONSTRUC     Construction
* 20_TRANSPORT    Land transport service(road rail) Water transport service Air transport service
...
/
 ...
 21_WTRP.         (wtp)
 22_ATRP.         (atp)
 23_SER.          (trd, afs, whs, cmn, ofi, ins, rsa, obs, ros, dwe)
 24_PUB.          (osg, edu, hht, wtr)
/
...
*==============================================================================
 ...
 CO_MB(i,z)      Household consumption of commodity I by source Import (basic price)
 CO_DB(i,z)      Household consumption of commodity I by source Domestic (basic price)
 CO_MBtax(i,z)   Household consumption of commodity I by source Import tax
 CO_DBtax(i,z)   Household consumption of commodity I by source Domestic tax
 CGO(i,z)        Public final consumption of commodity i
 CGO_MB(i,z)     Public final consumption of commodity i by source Import (basic price)
 CGO_DB(i,z)     Public final consumption of commodity i by source Domestic (basic price)
 CGO_MBtax(i,z)  Public final consumption of commodity i by source Import tax
 CGO_DBtax(i,z)  Public final consumption of commodity i by source Domestic tax
 INVO(i,z)       Final demand of commodity i for investment purposes
 INVO_MB(i,z)    Final demand of commodity i for investment purposes by source Import (basic price)
 INVO_DB(i,z)    Final demand of commodity i for investment purposes by source Domestic (basic price)
 INVO_MBtax(i,z) Final demand of commodity i for investment purposes by source Import tax
 INVO_DBtax(i,z) Final demand of commodity i for investment purposes by source Domestic tax
 DDO(i,z)        Demand for domestic production of commodity i
 DEPO(z)         Depreciation
 DIO(i,j,z)      Intermediate consumption of commodity i by industry j
 DIO_D(i,j,z)    Intermediate consumption of commodity i by industry j source D at basic price
 DIO_M(i,j,z)    Intermediate consumption of commodity i by industry j source I at basic price
...
* agents prices (VDPP) and private household imports at agents prices (VMPP).
 ...
 CO_MB(i,z)        = SUM{(trad_comm,reg)$[j2trad_comm(i,trad_comm)$z2reg(z,reg)],
                       VMPB(trad_comm,reg)};

 CO_DB(i,z)        = SUM{(trad_comm,reg)$[j2trad_comm(i,trad_comm)$z2reg(z,reg)],
                       VDPB(trad_comm,reg)};

 CO_MBtax(i,z)        = SUM{(trad_comm,reg)$[j2trad_comm(i,trad_comm)$z2reg(z,reg)],
                       VMPP(trad_comm,reg)-VMPB(trad_comm,reg)};

 CO_DBtax(i,z)        = SUM{(trad_comm,reg)$[j2trad_comm(i,trad_comm)$z2reg(z,reg)],
...
* agents prices (VDGP) and public final imports at agents prices (VMGP).
 ...
 CGO_MB(i,z)     = SUM{(trad_comm,reg)$[j2trad_comm(i,trad_comm)$z2reg(z,reg)],
                       VMGB(trad_comm,reg)};

 CGO_DB(i,z)     = SUM{(trad_comm,reg)$[j2trad_comm(i,trad_comm)$z2reg(z,reg)],
                       VDGB(trad_comm,reg)};

 CGO_MBtax(i,z)  = SUM{(trad_comm,reg)$[j2trad_comm(i,trad_comm)$z2reg(z,reg)],
                       VMGP(trad_comm,reg)-VMGB(trad_comm,reg)};

 CGO_DBtax(i,z)  = SUM{(trad_comm,reg)$[j2trad_comm(i,trad_comm)$z2reg(z,reg)],
...
*                      VDFP(trad_comm,'CGDS',reg)+VIFA(trad_comm,'CGDS',reg)};
 ...
 INVO_MB(i,z)       = SUM{(trad_comm,reg)$[j2trad_comm(i,trad_comm)$z2reg(z,reg)],
                      VMIB(trad_comm,reg)};

 INVO_DB(i,z)       = SUM{(trad_comm,reg)$[j2trad_comm(i,trad_comm)$z2reg(z,reg)],
                      VDIB(trad_comm,reg)};

 INVO_MBtax(i,z)    = SUM{(trad_comm,reg)$[j2trad_comm(i,trad_comm)$z2reg(z,reg)],
                      VMIP(trad_comm,reg)-VMIB(trad_comm,reg)};

 INVO_DBtax(i,z)    = SUM{(trad_comm,reg)$[j2trad_comm(i,trad_comm)$z2reg(z,reg)],
                      VDIP(trad_comm,reg)-VDIB(trad_comm,reg)};
...
* agents prices (VDFP) and public final imports at agents prices (VIFA).
 ...
 DIO_D(i,j,z)    = SUM[(trad_comm,trad_commj,reg)$[j2trad_comm(i,trad_comm)
                      $j2trad_comm(j,trad_commj)$z2reg(z,reg)],
                      VDFB(trad_comm,trad_commj,reg)];

 DIO_M(i,j,z)    = SUM[(trad_comm,trad_commj,reg)$[j2trad_comm(i,trad_comm)
                      $j2trad_comm(j,trad_commj)$z2reg(z,reg)],
                      VMFB(trad_comm,trad_commj,reg)];


 tssm(i,j,z)    = SUM[(trad_comm,trad_commj,reg)$[j2trad_comm(i,trad_comm)
                      $j2trad_comm(j,trad_commj)$z2reg(z,reg)],
                      VMFP(trad_comm,trad_commj,reg)-VMFB(trad_comm,trad_commj,reg)];

 tssd(i,j,z)    = SUM[(trad_comm,trad_commj,reg)$[j2trad_comm(i,trad_comm)
                      $j2trad_comm(j,trad_commj)$z2reg(z,reg)],
                      VDFP(trad_comm,trad_commj,reg)-VDFB(trad_comm,trad_commj,reg)];
...
* EXO(i,zj,z)     = SUM{(trad_comm,regj,reg)$[j2trad_comm(i,trad_comm)
*                       $z2reg(zj,regj)$z2reg(z,reg)],
*                       VXSB(trad_comm,regj,reg)};
...
* tmrg('20_LTRP',ij,zj,z)$IMO(ij,zj,z)
  tmrg('20_LTRP',ij,zj,z)$IMO(ij,zj,z)
                 = SUM{(trad_comm,regj,reg)$[j2trad_comm(ij,trad_comm)
                       $z2reg(zj,regj)$z2reg(z,reg)],
                       VTMFSD('otp',trad_comm,regj,reg)};

* tmrg('21_WTRP',ij,zj,z)$IMO(ij,zj,z)
  tmrg('21_WTRP',ij,zj,z)$IMO(ij,zj,z)
                 = SUM{(trad_comm,regj,reg)$[j2trad_comm(ij,trad_comm)
                       $z2reg(zj,regj)$z2reg(z,reg)],
                       VTMFSD('wtp',trad_comm,regj,reg)};

* tmrg('22_ATRP',ij,zj,z)$IMO(ij,zj,z)
 tmrg('22_ATRP',ij,zj,z)$IMO(ij,zj,z)
                 = SUM{(trad_comm,regj,reg)$[j2trad_comm(ij,trad_comm)
                       $z2reg(zj,regj)$z2reg(z,reg)],
                       VTMFSD('atp',trad_comm,regj,reg)};
...
* market prices (EVFB) and endowment at agent prices (EVOS)
 ...
 DTAX(f,z)          = SUM{(endw_comm,reg)$[f2endw_comm(f,endw_comm)$z2reg(z,reg)],
                       SUM[trad_comm,EVFB(endw_comm,trad_comm,reg)] -
                       SUM[trad_comm,EVOS(endw_comm, trad_comm, reg)]} ;
...
TIPO('21_WTRP','01_KOR') = -7020 ;
TIPO('10_CHEMICAL','05_MNG') = 1.00416E-9  ;
...
* aggregated acording to the share of each commodity (QO) TRAD_COMM in total I
 ...
 sigma_M1(i,z)   = SUM{(trad_comm,reg)$[j2trad_comm(i,trad_comm)$z2reg(z,reg)],
                   ESUBD(trad_comm,reg)*SH_Q(trad_comm,i,z)};
...
* aggregated acording to the share of total import (IMTO) TRAD_COMM in total I
 ...
 sigma_M2(i,z)   = SUM{(trad_comm,reg)$[j2trad_comm(i,trad_comm)$z2reg(z,reg)],
                   ESUBM(trad_comm,reg)*SH_IM(trad_comm,i,z)};
...
* acording to the share of each sector (VA) TRAD_COMM in total I
 ...
 sigma_VA(j,z)   = SUM{(trad_comm,reg)$[j2trad_comm(j,trad_comm)$z2reg(z,reg)],
                   ESUBVA(trad_comm,reg)*SH_VA(trad_comm,j,z)};

execute_unload 'DATA_AGG-2019_TRP_230111_MNG',
...
*Benchmark variables and parameters used in PEP w-1 and PEP w-t
 CO, CGO, DDO, DEPO, DIO, DIO_D, DIO_M, DSO, EXO, IMO, INVO, KSTO, LDO, MRGNO, POPO, RKDO,
 TDHO, DTAX, TICO, TIKO, TIMO, TIPO, TIWO, TIXO, tssm, tssd, tmrg, XSO, sigma_M1, sigma_M2,
 sigma_VA,Q_GTAP, SH_Q, ESUBD,  DIO2, TIMO2, IMO2, tmrg2, tmrg3, tmrg4, CO_MB, CO_DB,  CO_MBtax,  CO_DBtax
 CGO_MB, CGO_DB, CGO_MBtax, CGO_DBtax, INVO_MB, INVO_DB, INVO_MBtax, INVO_DBtax,
 DIO3, LDO2, TIWO2, TIKO2, RKDO2, RKDO2, RKDO_J, LDO_J, XSO_Matrix
;
...
* SAM Balancing
*==============================================================================
$INCLUDE DATA_SamBal-2019_TRP_230125.gms

*==============================================================================
* Importing WEB(World Energy Balance)
*==============================================================================
$INCLUDE DATA_WEB-2019_230131.gms
