...
*==============================================================================
Parameters
 CO2FACTOR(ene,j,z)     millions CO2 emissions factor in tens of billions (10G$)
 CTAX0(z)               initial Carbon tax $ per ton CO2
 TCTAX0(z)              initial Government Revenue from Carbon tax
 PERMITO(j,z)           initial Permit
 PPERMITO(z)            initial Permit Price  
 PERMIT_TOTALO(z)       initial Total Permit
;

 CO2FACTOR(ene,j,z) = 0 ;

* CO2FACTOR('02_COAL',j,z)$DEO('02_COAL',j,z) = sum(p_coal,CO2IO(p_coal,j,z))/ DEO('02_COAL',j,z) ;
 CO2FACTOR('04_GAS',j,z)$DEO('04_GAS',j,z) = (sum(p_gas,CO2IO(p_gas,j,z))/ DEO('04_GAS',j,z))/10000000 ;
...
* 18_ELEC         Electricity
 ...
 PPERMITO(Z) = 0.0000000001;
 CTAX0(z) = 0 ;
 TCTAX0(z) = 0;

execute_unload 'CO2FACTOR',
 CO2FACTOR, DEO, PERMITO, PERMIT_TOTALO ;
...
* TCTAX(z)           Government revenuse from Carbon tax
 TPERMIT(z)         Government revenuse from Emission Trading System
...
* EQ9_1(ene,j,z)..  DE(ene,j,z) =e= aij2(ene,j,z)*CE(j,z) ;
 ...
 EQ9_1(ene,j,z)..  DE(ene,j,z) =e= [beta_ENER(ene,j,z)*PCE(j,z)/((P4(ene,j,z)+PPERMIT(Z)*CO2FACTOR(ene,j,z)))]
                                   **sigma_ENER(j,z)*B_ENER(j,z)**(sigma_ENER(j,z)-1)
                                   *CE(j,z);
 
*EQENINP(sec).. ENINP(sec) =E= gamma_ENINP(sec)*KE(sec)*(PKE(sec)/(P("sec3")+PPERMIT*CO2FACTOR*(1-share_freePermit)))**sigma_KE(sec);
...
*    5.3.2.2 Government
*==============================================================================
 EQ16(z)..       YG(z) =e= TDH(z)+TPRODN(z)+TPRCTS(z)+TPERMIT(Z) ;

 EQ16_1(Z)..     TPERMIT(Z) =e= sum(j,PPERMIT(Z)*PERMIT(j,z));
...
*==============================================================================
 ...
 EQ51_1(j,z)..   PCE(j,z)*CE(j,z) =e= SUM[ene,PC(ene,z)*DE(ene,j,z) + PPERMIT(Z)*CO2FACTOR(ene,j,z)*DE(ene,j,z)];
...
*==============================================================================
 ...
 EQPERMIT(j,z).. PERMIT(j,z) =e= sum(ene, CO2FACTOR(ene,j,z)*DE(ene,j,z));
 
 EQMARKETP(z)..  sum(j,PERMIT(j,z))=E= PERMIT_TOTAL(z)  ;
*==============================================================================
...
*==============================================================================
 ...
 PERMIT.L(j,z)       = PERMITO(j,z) ;
 PERMIT_TOTAL.L(z)   = PERMIT_TOTALO(z) ;
...
*==============================================================================
 ...
 PERMIT_TOTAL.FX(z) = PERMIT_TOTALO(z);
* CTAX.FX(z)       = CTAX0(Z);
*==============================================================================
*  6.4 Simulation
*==============================================================================
...
PERMIT_TOTAL.FX('01_KOR') = PERMIT_TOTALO('01_KOR')*0.8 ;
* PERMIT_TOTAL.FX(z) = PERMIT_TOTALO(z)*0.8 ;
...
$INCLUDE RESULTS_PEP-w-1_v4.0_231208_permit.gms
