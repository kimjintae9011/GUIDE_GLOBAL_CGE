...
/
 ...
 17_ELEC         Electricity
 18_CONSTRUC     Construction
 19_LTRP         Land transport service(road rail)
 20_WTRP         Water transport service
 21_ATRP         Air transport service
 22_SER          Service
/
...
* marg_comm(prod_comm)  Margin commodities
 ...
 trad_comm(GlobalSet)  All sectors and commodities except for investment
 marg_comm(trad_comm)  Margin commodities
...
j2prod_comm(j,prod_comm) Mapping of sectors
/
 ...
 20_WTRP.         (wtp)
 21_ATRP.         (atp)
 22_SER.          (trd, afs, whs, cmn, ofi, ins, rsa, obs, ros, dwe, osg, edu, hht, wtr)
/

j3prod_comm(j,trad_comm) Mapping of sectors
/
 01_AGRICULT.     (pdr, wht, gro, v_f, osd, c_b, pfb, ocr, ctl, oap, rmk, wol, frs, fsh)
 02_COAL.         (coa)
 03_OIL.          (oil)
...
j3trad_comm(i,trad_comm) Mapping of sectors
/
 01_AGRICULT.     (pdr, wht, gro, v_f, osd, c_b, pfb, ocr, ctl, oap, rmk, wol, frs, fsh)
 02_COAL.         (coa)
 03_OIL.          (oil)
...
/
 ...
 08_WOODPAPER.    (08_WOODPRO, 09_PAPERPRO)
 09_PETROLCOAL.   (10_PETROLCOAL)
 10_CHEMICAL.     (11_CHEMICAL)
 11_NONMET.       (12_NONMET)
 12_IRONSTL.      (13_IRONSTL)
 13_NONFERR.      (14_NONFERR)
 14_MACHINE.      (15_MACHINE)
 15_TRANSEQ.      (16_TRANSEQ)
 16_OTHERIND.     (17_OTHERIND)
 17_ELEC.         (18_ELEC)
 18_CONSTRUC.     (19_CONSTRUC)
 19_LTRP.         (20_LTRP)
 20_WTRP.         (21_WTRP)
 21_ATRP.         (22_ATRP)
 22_SER.          (23_SER)
...
ALIAS(reg,regj)
ALIAS(trad_comm,trad_commi)
ALIAS(prod_comm,prod_commj)
ALIAS(j,jj)
ALIAS(i,ij)
ALIAS(l,lj)
ALIAS(k,kj)
...
* of USD
 ...
 MAKES(trad_comm,prod_comm,reg)   Make matrix valued at suppliers prices
 MAKEB(trad_comm,prod_comm,reg)   Make matrix valued at basic prices
...
* agents prices (VDPP) and private household imports at agents prices (VMPP).
 ...
 CO(i,z)         = SUM{(trad_comm,reg)$[j3trad_comm(i,trad_comm)$z2reg(z,reg)],
...
* agents prices (VDGP) and public final imports at agents prices (VMGP).
 ...
 CGO(i,z)        = SUM{(trad_comm,reg)$[j3trad_comm(i,trad_comm)$z2reg(z,reg)],
...
*  and public final imports at agents prices (VIFA) for this sector.
 ...
 INVO(i,z)       = SUM{(trad_comm,reg)$[j3trad_comm(i,trad_comm)$z2reg(z,reg)],
...
* agents prices (VDFP) and public final imports at agents prices (VIFA).
 ...
 DIO(i,j,z)      = SUM[(trad_comm,prod_comm,reg)$[j3trad_comm(i,trad_comm)
                      $j2prod_comm(j,prod_comm)$z2reg(z,reg)],
                      VDFP(trad_comm,prod_comm,reg)
                     +VMFP(trad_comm,prod_comm,reg)];

 tssm(i,j,z)    = SUM[(trad_comm,prod_comm,reg)$[j3trad_comm(i,trad_comm)
                      $j2prod_comm(j,prod_comm)$z2reg(z,reg)],
                      VMFP(trad_comm,prod_comm,reg)-VMFB(trad_comm,prod_comm,reg)];

 tssd(i,j,z)    = SUM[(trad_comm,prod_comm,reg)$[j3trad_comm(i,trad_comm)
                      $j2prod_comm(j,prod_comm)$z2reg(z,reg)],
                      VDFP(trad_comm,prod_comm,reg)-VDFB(trad_comm,prod_comm,reg)];
...
** is removed.
 ...
 IMO(i,zj,z)     = SUM{(trad_comm,regj,reg)$[j3trad_comm(i,trad_comm)
...
* tmrg('20_LTRP',ij,zj,z)$IMO(ij,zj,z)
  tmrg('20_LTRP',ij,zj,z)$IMO(ij,zj,z)
                 = SUM{(trad_comm,regj,reg)$[j3trad_comm(ij,trad_comm)
                       $z2reg(zj,regj)$z2reg(z,reg)],
...
* Supply of transport margin are given by the variable VST
 MRGNO(i,z)      = SUM{(marg_comm,reg)$[j3trad_comm(i,marg_comm)$z2reg(z,reg)],
                       VST(marg_comm,reg)};
...
*  Import duties are given by the variable TFRV
 ...
 TIMO(i,zj,z)$IMO(i,zj,z)
                 = SUM{(trad_comm,regj,reg)$[j3trad_comm(i,trad_comm)
                       $z2reg(zj,regj)$z2reg(z,reg)],
...
* TIXO(i,z,zj)$IMO(i,z,zj)
 TIXO(i,z,zj)$IMO(i,z,zj)
                 = SUM{(trad_comm,reg,regj)$[j3trad_comm(i,trad_comm)
...
* less subsidies (FBEP).
 ...
 TIWO(l,j,z)      = SUM{(endw_comm,prod_comm,reg)$[f2endw_comm(l,endw_comm)
                       $j2prod_comm(j,prod_comm)$z2reg(z,reg)],
                       FTRV(endw_comm,prod_comm,reg)+
                       FBEP(endw_comm,prod_comm,reg)};

 TIWO_J(l,z) = SUM(j, TIWO(l,j,z));

 TIKO(k,j,z)     = SUM{(endw_comm,prod_comm,reg)$[f2endw_comm(k,endw_comm)
                       $j2prod_comm(j,prod_comm)$z2reg(z,reg)],
                       FTRV(endw_comm,prod_comm,reg)+
                       FBEP(endw_comm,prod_comm,reg)};
...
* subsidies (OSEP).
 ...
 TIPO(j,z)       = SUM{(trad_comm,reg)$[j3prod_comm(j,trad_comm)$z2reg(z,reg)],
                       OSEP(trad_comm,reg)};
...
* demand at agent prices and demand at market prices.
 ...
 TICO(i,z)       = SUM{(trad_comm,reg)$[j3trad_comm(i,trad_comm)$z2reg(z,reg)],
...
* market prices (EVFB) and endowment at agent prices (EVOS)
 ...
 DTAX(f,z)          = SUM{(endw_comm,reg)$[f2endw_comm(f,endw_comm)$z2reg(z,reg)],
                       SUM[prod_comm,EVFB(endw_comm,prod_comm,reg)] -
                       SUM[prod_comm,EVOS(endw_comm, prod_comm, reg)]} ;
...
* that is before taxes (EVFB).
 ...
 LDO(l,j,z)      = SUM{(endw_comm,prod_comm,reg)$[f2endw_comm(l,endw_comm)
                       $j2prod_comm(j,prod_comm)$z2reg(z,reg)],
                       EVFB(endw_comm,prod_comm,reg)};

 LDO_J(l,z)       = SUM(j,LDO(l,j,z)) - DTAX(l,z);

 RKDO(k,j,z)      = SUM{(endw_comm,prod_comm,reg)$[f2endw_comm(k,endw_comm)
                       $j2prod_comm(j,prod_comm)$z2reg(z,reg)],
                       EVFB(endw_comm,prod_comm,reg)};
...
*==============================================================================
 ...
 MAKE(j,i,z) = SUM[(prod_comm,trad_comm,reg)$[j3trad_comm(i,trad_comm)
                      $j2prod_comm(j,prod_comm)$z2reg(z,reg)],
                      MAKEB(trad_comm,prod_comm,reg)];
...
* XSTO(j,z)         = SUM(i, XSO(j,i,z));
 ...
 DDO_Matrix(i,ij,z)  = DDO(i,z)$sameas(i,ij);
 DSO(j,i,z)      = SUM{(ij)$[COMtoIND(j,i)],DDO_Matrix(i,ij,z)};
 DSO_I(i,z)      = SUM(j, DSO(j,i,z));
 XSO_I(i,z)      = DDO(i,z)+MRGNO(i,z)+SUM[zj,EXO(i,z,zj)-TIXO(i,z,zj)];
*
 XSO_Matrix(i,ij,z) =  XSO_I(i,z)$sameas(i,ij);
 XSO(j,i,z)      = SUM{(ij)$[COMtoIND(j,i)],XSO_Matrix(i,ij,z)};

 XSTO(j,z)       = SUM(i, XSO(j,i,z));
 EXO2(j,i,z)     = XSO(j,i,z) - DSO(j,i,z);
...
*==============================================================================
 ...
 SH_VA(prod_comm,j,z) Share of each sector TRAD_COM in aggregation j for region z
 VA_GTAP(prod_comm,z) Value added using the GTAP sectors
;
...
* aggregated acording to the share of each commodity (QO) TRAD_COMM in total I
 ...
 SH_Q(trad_comm,i,z)$j3trad_comm(i,trad_comm)
                 = Q_GTAP(trad_comm,z)/
                   SUM{trad_commi$j3trad_comm(i,trad_commi),
                       Q_GTAP(trad_commi,z)};

 sigma_M1(i,z)   = SUM{(trad_comm,reg)$[j3trad_comm(i,trad_comm)$z2reg(z,reg)],
...
* aggregated acording to the share of total import (IMTO) TRAD_COMM in total I
 ...
 SH_IM(trad_comm,i,z)$j3trad_comm(i,trad_comm)
                 = IM_GTAP(trad_comm,z)/
                   SUM{trad_commi$j3trad_comm(i,trad_commi),
                       IM_GTAP(trad_commi,z)};

 sigma_M2(i,z)   = SUM{(trad_comm,reg)$[j3trad_comm(i,trad_comm)$z2reg(z,reg)],
...
* acording to the share of each sector (VA) TRAD_COMM in total I
 ...
 VA_GTAP(prod_comm,z)
                 = SUM{endw_comm,
                       SUM[reg$z2reg(z,reg),EVFB(endw_comm,prod_comm,reg)]};

 SH_VA(prod_comm,j,z)$j2prod_comm(j,prod_comm)
                 = VA_GTAP(prod_comm,z)/
                   SUM{prod_commj$j2prod_comm(j,prod_commj),
                       VA_GTAP(prod_commj,z)};

 sigma_VA(j,z)   = SUM{(prod_comm,reg)$[j2prod_comm(j,prod_comm)$z2reg(z,reg)],
                   ESUBVA(prod_comm,reg)*SH_VA(prod_comm,j,z)};

*$exit

execute_unload 'JointA_DATA_AGG-2019_230111_MNG',
...
*Benchmark variables and parameters used in PEP w-1 and PEP w-t
 CO, CGO, DDO, DEPO, DIO, DSO,DSO_I, EXO, EXO2, IMO, INVO, KSTO, LDO, MRGNO, POPO, RKDO,
