...
* CTAX.fX(CTAX_Z,time)                     = CTAX0(CTAX_Z);
 PERMIT_TOTAL.fx(PERMIT2_Z,time)   = PERMIT_TOTALO(PERMIT2_Z) ;
 PERMIT_NEA_TOTAL.fx(time)             = SUM(NEA, PERMIT_TOTALO(NEA));
...
*                            = CTAX_NZS(z,time); 
 ...
 PERMIT_TOTAL.fx(PERMIT2_Z,time)$[ord(time) gt 1]
                             = PERMIT_TOTALO(PERMIT2_Z)*PERMIT_NZS(PERMIT2_Z,time);  

 PERMIT_NEA_TOTAL.fx(time)$[ord(time) gt 1]
                             = SUM(NEA,PERMIT_TOTALO(NEA))*PERMIT_NEA_NZS('01_KOR',time);  
...
*============================================================================== 
* NEA
CTAX_CO2(ene,j,NEA,time)$[ord(time) gt 2]  =(1+ CTAX_NEA.L(time))** gamma_CO2(ene,j,NEA);

CO2FACTOR2_Star(ene,j,NEA,time)$[ord(time) gt 2]= 
CO2FACTOR(ene,j,NEA) * exp(
 alpha_CO2(ene,j,NEA) - alpha_CO2(ene,j,NEA) *CTAX_CO2(ene,j,NEA,time)
 );

CO2FACTOR2(ene,j,NEA,time)$[ord(time) gt 2]= CO2FACTOR2(ene,j,NEA,time-1)+0.1*(CO2FACTOR2_Star(ene,j,NEA,time)-CO2FACTOR2(ene,j,NEA,time-1));

*Other
CTAX_CO2(ene,j,PERMIT2_Z,time)$[ord(time) gt 2]  =(1+ CTAX.L(PERMIT2_Z,time))** gamma_CO2(ene,j,PERMIT2_Z);

CO2FACTOR2_Star(ene,j,PERMIT2_Z,time)$[ord(time) gt 2]= 
CO2FACTOR(ene,j,PERMIT2_Z) * exp(
 alpha_CO2(ene,j,PERMIT2_Z) - alpha_CO2(ene,j,PERMIT2_Z) *CTAX_CO2(ene,j,PERMIT2_Z,time)
 );

CO2FACTOR2(ene,j,PERMIT2_Z,time)$[ord(time) gt 2]= CO2FACTOR2(ene,j,PERMIT2_Z,time-1)+0.1*(CO2FACTOR2_Star(ene,j,PERMIT2_Z,time)-CO2FACTOR2(ene,j,PERMIT2_Z,time-1));


*==============================================================================
* Backstop technologies
*==============================================================================
penetration_rate(i3,NEA,time)$[CTAX_NEA.L(time) gt 1.0] = penetration_rate(i3,NEA,time-1)+0.03;
penetration_rate(i3,PERMIT2_Z,time)$[CTAX.L(PERMIT2_Z,time) gt 1.0] = penetration_rate(i3,PERMIT2_Z,time-1)+0.03;


if ((CTAX_NEA.L(time) gt 1.0), switch(i3,'01_KOR',time) = 1  ;
else switch(i3,'01_KOR',time) = 0 ;
);

if ((CTAX_NEA.L(time) gt 1.0), switch(i3,'02_CHN',time) = 1  ;
else switch(i3,'02_CHN',time) = 0 ;
);

if ((CTAX_NEA.L(time) gt 1.0), switch(i3,'03_JPN',time) = 1  ;
else switch(i3,'03_JPN',time) = 0 ;
);

if ((CTAX_NEA.L(time) gt 1.0), switch(i3,'04_RUS',time) = 1  ;
else switch(i3,'04_RUS',time) = 0 ;
);

if ((CTAX_NEA.L(time) gt 1.0), switch(i3,'05_MNG',time) = 1  ;
else switch(i3,'05_MNG',time) = 0 ;
);

if ((CTAX_NEA.L(time) gt 1.0), switch(i3,'06_PRK',time) = 1  ;
