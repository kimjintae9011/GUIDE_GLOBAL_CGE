...
$GDXIN DATA_AGG-2019_230111_MNG.gdx
*$GDXIN SAM-BAL-2019_230217_MNG.gdx

$LOAD J, I1, BUS, PUB, F, L, K, ENE, TFC, Z, ZR, Z1, Zrich, Zother
...
*==============================================================================
*  2.1.1 Parameters (energy)
*==============================================================================
 EEI(ENE,j,z)      Energy demand by production sectos by fuel
 NEEI(ENE,j,z)     Non-Energy Demand by production sectos by fuel
 EEH(ENE,z)        Energy demand by household by fuel
...
$CALL GDXXRW.EXE VAL_230217_PAR.xls squeeze = 'no' par=sigma_KD rng=PAR!A5:R28 par=sigma_LD rng=PAR!A31:R54 par=sigma_X1 rng=PAR!A57:R80 par=sigma_X2 rng=PAR!A83:R106 par=sigma_y rng=PAR!A109:R132 par=PARZ rng=PAR!A134:C151 par=sigma_inv rng=PAR!A154:R177 Rdim=2 Cdim=1

$GDXIN VAL_230217_PAR.gdx
...
display rho_X1, beta_EX_X1, beta_D_X1, B_X1, rho_X2, beta_X2, B_X2 ;
*$exit
*==============================================================================
...
*==============================================================================
*  4.9 Energy Consumptionb Coefficient
*==============================================================================
$INCLUDE DATA_WEB-2019_230131.gms

 EEI('02_COAL',j,z) = sum(p_coal, EC_DIO(p_coal,j,z))/DIO('02_COAL',j,z);
 NEEI('02_COAL',j,z) = sum(p_coal, NEC_DIO(p_coal,j,z))/DIO('02_COAL',j,z);

 EEH('02_COAL',z)   = sum(p_coal, EC_CO(p_coal,z))/CO('02_COAL',z);

 EEI('04_GAS',j,z) = sum(p_gas, EG_DIO(p_gas,j,z))/DIO('04_GAS',j,z);
 NEEI('04_GAS',j,z) = sum(p_gas, NEG_DIO(p_gas,j,z))/DIO('04_GAS',j,z);
 EEH('04_GAS',z)   = sum(p_gas, EG_CO(p_gas,z))/CO('04_GAS',z);

 EEI('10_PETROLCOAL',j,z) = sum(p_oilproduct, EP_DIO(p_oilproduct,j,z))/DIO('10_PETROLCOAL',j,z);
 NEEI('10_PETROLCOAL',j,z) = sum(p_oilproduct, NEP_DIO(p_oilproduct,j,z))/DIO('10_PETROLCOAL',j,z);
 EEH('10_PETROLCOAL',z)   = sum(p_oilproduct, EP_CO(p_oilproduct,z))/CO('10_PETROLCOAL',z);

 EEI('18_ELEC',j,z) = sum(p_elec, EE_DIO(p_elec,j,z))/DIO('18_ELEC',j,z);
 EEH('18_ELEC',z)   = sum(p_elec, EE_CO(p_elec,z))/CO('18_ELEC',z);
...
*ttim.FX(i,zj,z) = ttimO(i,zj,z)*0.5;
ttic.FX('08_WOODPRO','01_KOR') = tticO('08_WOODPRO','01_KOR')*1.5;
