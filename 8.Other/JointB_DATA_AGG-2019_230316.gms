...
/
 ...
 15_MACHINE      Fabricated metal products Electronic and electrical equipment Machinery and equipment
 16_ELEC         Electricity
 17_CONSTRUC     Construction
 18_LTRP         Land transport service(road rail)
 19_WTRP         Water transport service
 20_ATRP         Air transport service
 21_SER          Service
/
...
i2trad_comm(i,trad_comm) Mapping of sectors
/
 01_AGRICULT.     (pdr, wht, gro, v_f, osd, c_b, pfb, ocr, ctl, oap, rmk, wol, frs, fsh)
 02_COAL.         (coa)
 03_OIL.          (oil)
 ...
 13_IRONSTL.      (i_s)
 14_NONFERR.      (nfm)
 15_MACHINE.      (fmp, ele, eeq, ome, mvh, otn, omf)
 16_ELEC.         (ely)
 17_CONSTRUC.     (cns)
 18_LTRP.         (otp)
 19_WTRP.         (wtp)
 20_ATRP.         (atp)
 21_SER.          (trd, afs, whs, cmn, ofi, ins, rsa, obs, ros, dwe, osg, edu, hht, wtr)
/
...
/
 ...
 16_TRANSEQ.      (15_MACHINE)
 17_OTHERIND.     (15_MACHINE)
 18_ELEC.         (16_ELEC)
 19_CONSTRUC.     (17_CONSTRUC)
 20_LTRP.         (18_LTRP)
 21_WTRP.         (19_WTRP)
 22_ATRP.         (20_ATRP)
 23_SER.          (21_SER)
...
* agents prices (VDPP) and private household imports at agents prices (VMPP).
 ...
 CO(i,z)         = SUM{(trad_comm,reg)$[i2trad_comm(i,trad_comm)$z2reg(z,reg)],
...
* agents prices (VDGP) and public final imports at agents prices (VMGP).
 ...
 CGO(i,z)        = SUM{(trad_comm,reg)$[i2trad_comm(i,trad_comm)$z2reg(z,reg)],
...
*  and public final imports at agents prices (VIFA) for this sector.
 ...
 INVO(i,z)       = SUM{(trad_comm,reg)$[i2trad_comm(i,trad_comm)$z2reg(z,reg)],
...
* agents prices (VDFP) and public final imports at agents prices (VIFA).
 ...
 DIO(i,j,z)      = SUM[(trad_comm,prod_comm,reg)$[i2trad_comm(i,trad_comm)
                      $j2prod_comm(j,prod_comm)$z2reg(z,reg)],
                      VDFP(trad_comm,prod_comm,reg)
                     +VMFP(trad_comm,prod_comm,reg)];

 tssm(i,j,z)    = SUM[(trad_comm,prod_comm,reg)$[i2trad_comm(i,trad_comm)
                      $j2prod_comm(j,prod_comm)$z2reg(z,reg)],
                      VMFP(trad_comm,prod_comm,reg)-VMFB(trad_comm,prod_comm,reg)];

 tssd(i,j,z)    = SUM[(trad_comm,prod_comm,reg)$[i2trad_comm(i,trad_comm)
                      $j2prod_comm(j,prod_comm)$z2reg(z,reg)],
...
** is removed.
 ...
 IMO(i,zj,z)     = SUM{(trad_comm,regj,reg)$[i2trad_comm(i,trad_comm)
...
*  Import duties are given by the variable TFRV
 ...
 TIMO(i,zj,z)$IMO(i,zj,z)
                 = SUM{(trad_comm,regj,reg)$[i2trad_comm(i,trad_comm)
                       $z2reg(zj,regj)$z2reg(z,reg)],
...
* TIXO(i,z,zj)$IMO(i,z,zj)
 TIXO(i,z,zj)$IMO(i,z,zj)
                 = SUM{(trad_comm,reg,regj)$[i2trad_comm(i,trad_comm)
...
* demand at agent prices and demand at market prices.
 ...
 TICO(i,z)       = SUM{(trad_comm,reg)$[i2trad_comm(i,trad_comm)$z2reg(z,reg)],
...
*==============================================================================
 ...
 MAKE(j,i,z) = SUM[(prod_comm,trad_comm,reg)$[i2trad_comm(i,trad_comm)
                      $j2prod_comm(j,prod_comm)$z2reg(z,reg)],
...
* DSO(j,i,z)$XSO(j,i,z) = XSO(j,i,z)$XSO(j,i,z) - EXTO(i,z)$EXTO(i,z) - MRGNO(i,z)$MRGNO(i,z);
...
*execute_unload 'JointB',
*DDO, DDO_Matrix, DSO, MAKE, XSO_I, XSO, XSTO ;
*$EXIT
...
* aggregated acording to the share of each commodity (QO) TRAD_COMM in total I
 ...
 SH_Q(trad_comm,i,z)$i2trad_comm(i,trad_comm)
                 = Q_GTAP(trad_comm,z)/
                   SUM{trad_commi$i2trad_comm(i,trad_commi),
                       Q_GTAP(trad_commi,z)};

 sigma_M1(i,z)   = SUM{(trad_comm,reg)$[i2trad_comm(i,trad_comm)$z2reg(z,reg)],
...
* aggregated acording to the share of total import (IMTO) TRAD_COMM in total I
 ...
 SH_IM(trad_comm,i,z)$i2trad_comm(i,trad_comm)
                 = IM_GTAP(trad_comm,z)/
                   SUM{trad_commi$i2trad_comm(i,trad_commi),
                       IM_GTAP(trad_commi,z)};

 sigma_M2(i,z)   = SUM{(trad_comm,reg)$[i2trad_comm(i,trad_comm)$z2reg(z,reg)],
...
execute_unload 'JointB_DATA_AGG-2019_230111_MNG',
