...
;
 ...
 ttip_shareO(j,z) = XSTO(j,z)/sum(jj, XSTO(j,z));
 deltattipO(j,z)  = -ttip_shareO(j,z)*TCBAMO(z)/(XSTO(j,z)*PPO(j,z));

 ttiw_shareO(l,j,z) = LDO(l,j,z)/SUM((lj,jj),LDO(l,jj,z));
 deltattiwO(l,j,z)  = -ttiw_shareO(l,j,z)*TCBAMO(z)/WO(l,z)*LDO(l,j,z);
...
*==============================================================================
*  5.1.6. Tax rebate variables
*==============================================================================
 deltattip(j,z)     Production tax reduction due to subsidies in industry j
 ttip_share(j,z)    industry output share
 
 deltattiw(l,j,z)     labor tax reduction due to subsidies in industry j 
 ttiw_share(l,j,z)    labor share
;

*S1
 deltattip.fx(j,z)$(not (eNFs(j) and z5(z))) = 0;
 ttip.fx(j,z)$(not (eNFs(j) and z5(z))) = ttipO(j,z);

*S2
*deltattiw.fx(l,j,z)$(not (J11(j) and z5(z))) = 0;
*ttiw.fx(l,j,z)$(not (J11(j) and z5(z))) = ttiwO(l,j,z); 
*==============================================================================
...
* EQ10(z)            Household total income
 ...
 EQ10_1(z4)         Household total income in Non-EU z4 countries
 EQ10_2(z5)         Household total income in Non-EU z5 countries
...
* EQB_5(j,i,z)      Supply of exports for compsite activity
 ...
 EQB_5(j,i,z)       Supply of exports for compsite activity
* EQ94(z)            Utility of region z
* EQ95(z)            expenditure function of region z  
 EQ96(j,z)          S1: Industry share of j in z
 EQ97(j,z)          S1: Final production tax rate (adding subsidies)
 EQ98(j,z)          S1: Tax reduction
* EQ99(l,j,z)        S2:
* EQ100(l,j,z)       S2: 
* EQ101(l,j,z)       S2:  
;
...
* EQ10_2(z5)..    YH(z5) =e= YHL(z5)+YHK(z5)+TCTAX(z5);  

*S1
 EQ10(z3)..      YH(z3) =e= YHL(z3)+YHK(z3)+TCTAX(z3);
 EQ10_1(z4)..    YH(z4) =e= YHL(z4)+YHK(z4)+TCTAX(z4);   
 EQ10_2(z5)..    YH(z5) =e= YHL(z5)+YHK(z5)+TCTAX(z5);  

*s3:lump-sum transfer to z5 hh
* EQ10(z3)..      YH(z3) =e= YHL(z3)+YHK(z3)+TCTAX(z3);
* EQ10_1(z4)..    YH(z4) =e= YHL(z4)+YHK(z4)+TCTAX(z4);   
* EQ10_2(z5)..    YH(z5) =e= YHL(z5)+YHK(z5)+TCTAX(z5)+TCBAM('07_EU');  
...
*==============================================================================
 ...
 EQ16_6(z5)..    TCBAM(z5) =e= 0;
 EQ16_4(z3)..    TCBAM(z3) =e= sum((i,zj), CP(zj)*CO2IDFACTOR(i,zj)*e(z3)*IM(i,zj,z3)*PWM(i,zj,z3));

****CTAX + CBAM baseline: CBAM w/o any complimentary scenarios****
...
* EQ16_5(z5)..    YG(z5)    =e= TDH(z5)+TPRODN(z5)+TPRCTS(z5)+TCTAX(z5); 

***** CBAM revenue recycling: s1. renewable energy production subsidies for z5 *****
 EQ16(z4)..      YG(z4)   =e= TDH(z4)+TPRODN(z4)+TPRCTS(z4)+TCTAX(z4); 
 EQ16_1(z)..     TCTAX(z) =e= sum((ene,j), PC(ene,z)*CTAX(z)*CO2FACTOR(ene,j,z)*DE(ene,j,z));
 EQ16_2(z3)..    YG(z3)   =e= TDH(z3)+TPRODN(z3)+TPRCTS(z3)+TCTAX(z3);
 EQ16_5(z5)..    YG(z5)   =e= TDH(z5) + TPRODN(z5) + TPRCTS(z5) + TCTAX(z5) + TCBAM('07_EU')/card(z5);

***** CBAM revenue recycling: s2. just transtion: labor tax subsidies for z5 *****
* EQ16(z4)..      YG(z4)   =e= TDH(z4)+TPRODN(z4)+TPRCTS(z4)+TCTAX(z4); 
* EQ16_1(z)..     TCTAX(z) =e= sum((ene,j), PC(ene,z)*CTAX(z)*CO2FACTOR(ene,j,z)*DE(ene,j,z));
* EQ16_2(z3)..    YG(z3)   =e= TDH(z3)+TPRODN(z3)+TPRCTS(z3)+TCTAX(z3)+ TCBAM(z3) - sum(z5, TCBAM('07_EU')/card(z5));
* EQ16_5(z5)..    YG(z5)   =e= TDH(z5) + TPRODN(z5) + TPRCTS(z5) + TCTAX(z5) + TCBAM('07_EU')/card(z5);

****CBAM revenue recycling: s3. lump-sum transfer to z5 HH****
* EQ16(z4)..      YG(z4)    =e= TDH(z4)+TPRODN(z4)+TPRCTS(z4)+TCTAX(z4);
* EQ16_1(z)..     TCTAX(z)  =e= sum((ene,j), PC(ene,z)*CTAX(z)*CO2FACTOR(ene,j,z)*DE(ene,j,z));
* EQ16_2(z3)..    YG(z3)    =e= TDH(z3)+TPRODN(z3)+TPRCTS(z3)+TCTAX(z3)+ TCBAM(z3);
* EQ16_5(z5)..    YG(z5)    =e= TDH(z5)+TPRODN(z5)+TPRCTS(z5)+TCTAX(z5); 
...
* EQ32_2(z5)..     SG(z5) =e= YG(z5)-G(z5)-TCTAX(z5);

***** CBAM revenue recycling: s1. renewable energy production subsidies for z5 & s2.Just transition *****
 EQ32(z3)..       SG(z3) =e= YG(z3)-G(z3)-TCTAX(z3);
 EQ32_1(z4)..     SG(z4) =e= YG(z4)-G(z4)-TCTAX(z4);
 EQ32_2(z5)..     SG(z5) =e= YG(z5)-G(z5)-TCTAX(z5);

*****CTAX + CBAM baseline: lump-sum transfer to EU hh *****
* EQ32(z3)..       SG(z3) =e= YG(z3)-G(z3)-TCTAX(z3);
* EQ32_1(z4)..     SG(z4) =e= YG(z4)-G(z4)-TCTAX(z4)- TCBAM(z4);
* EQ32_2(z5)..     SG(z5) =e= YG(z5)-G(z5)-TCTAX(z5)- TCBAM(z5);
...
* EQ82(z)..       GDP_MP(z) =e= GDP_BP(z)+TPRCTS(z)+TCTAX(z)+TCBAM(z);

* EQ83(z)..       GDP_IB(z) =e= SUM[(l,j)$LDO(l,j,z),W(l,z)*LD(l,j,z)]
*                              +SUM[(k,j)$KDO(k,j,z),R(k,j,z)*KD(k,j,z)]
*                              +TPRCTS(z)+TCTAX(z)+TCBAM(z)+TPRODN(z);
...
*                    *(PWM(i,zj,z)+SUM[ij,PWMG(ij)*tmrg(ij,i,zj,z)])]-TCBAM(z);

 EQ82(z)..       GDP_MP(z) =e= GDP_BP(z)+TPRCTS(z)+TCTAX(z);

 EQ83(z)..       GDP_IB(z) =e= SUM[(l,j)$LDO(l,j,z),W(l,z)*LD(l,j,z)]
                              +SUM[(k,j)$KDO(k,j,z),R(k,j,z)*KD(k,j,z)]
                              +TPRCTS(z)+TCTAX(z)+TPRODN(z);
...
*==============================================================================
*   5.3.10 CBAM Tax rebate:s1&s2
*==============================================================================
 EQ96(j,z)$(eNFs(j) and z5(z))  .. ttip_share(j,z) =e= XST(j,z)*PP(j,z) / sum(jj$(eNFs(jj)),XST(jj,z)*PP(jj,z));
 EQ97(j,z)$(eNFs(j) and z5(z))  .. ttip(j,z)       =e= ttipO(j,z) + deltattip(j,z);
 EQ98(j,z)$(eNFs(j) and z5(z))  .. deltattip(j,z)  =e= -ttip_share(j,z)*(TCBAM('07_EU')/card(z5))/(XST(j,z)*PP(j,z));

* EQ99(l,j,z)$(J11(j) and z5(z))   .. ttiw_share(l,j,z)  =e= LD(l,j,z)/SUM((lj,jj),LD(l,jj,z));
* EQ100(l,j,z)$(J11(j) and z5(z))  .. ttiw(l,j,z)        =e= ttiwO(l,j,z) + deltattiw(l,j,z);
* EQ101(l,j,z)$(J11(j) and z5(z))  .. deltattiw(l,j,z)   =e= -ttiw_share(l,j,z)*(TCBAM('07_EU')/card(z5))/W(l,z)*LD(l,j,z);
...
* ep.l(z)          = epO(z);
* UU.l(z)          = UUO(z);
* ttip_share.l(j,z) =  ttip_shareO(j,z);
 deltattip.l(j,z)   = deltattipO(j,z);
...
MODEL CBAM static CABM model /all/ ;
CBAM.holdfixed=1;
CBAM.solvelink=5;
CBAM.scaleopt=1;
...
*SOLVE CBAM USING MCP;
*SOLVE CBAM USING nlp MAXIMIZING OBJ;
...
loop(sim,
    if(ord(sim) = 1,    
       ...
       CP.FX('07_EU')  = 0;       
       CP.FX('01_KOR') = 40 + (ord(sim) -1)*10;
       CP.FX('02_CHN') = 40 + (ord(sim) -1)*10;
       CP.FX('03_JPN') = 40 + (ord(sim) -1)*10;
       CP.FX('04_IND') = 40 + (ord(sim) -1)*10;
       CP.FX('05_RUS') = 40 + (ord(sim) -1)*10;;
       CP.FX('06_USA') = 40 + (ord(sim) -1)*10;
       CP.FX('08_OHI') = 40 + (ord(sim) -1)*10;
       CP.FX('09_OUMIC') = 40 + (ord(sim) -1)*10;
       CP.FX('10_OLMIC') = 40 + (ord(sim) -1)*10;
       CP.FX('11_OLI') = 40 + (ord(sim) -1)*10;
           );
        SOLVE CBAM USING CNS;
*$offText
...
execute_unload 'Output_w-1/Results.gdx'
 valgamma_LES, valTCBAM, valGDP_BP, valGDP_BP_REAL, valPIXGDP, valCTAX, 
 valCP, valTCTAX, valSH, valKLE, valPC, valC, valCAB, valCABX, valCG, 
 valCI, valCE, valCMIN, valCTH, valCTH_REAL, valDD, valDEP, valDI, valDE, 
 valDIT, valDS, valDS_I, vale, valEX, valEXT, valG, valG_REAL, valGDP_BP_W, 
 valGDP_BP_W_REAL, valGDP_FD, valGDP_IB, valGDP_MP, valGDP_MP_REAL, valIM, 
 valIMT, valINV, valIT, valIT_REAL, valKD, valKDC, valKS, valLD, valLDC, 
 valLS, valMRGN, valP, valPT, valP2, valP4, valPCI, valPCE, valPD, valPE, 
 valPET, valPIXCON, valPIXGDP_W, valPIXGVT, valPIXINV, valPK, valPL, valPM, 
 valPMT, valPP, valPVA, valPKLE, valPWM, valPWMG, valPWX, valQ, valR, valRC, 
 valRK, valRTI, valSG, valsh0, valsh1, valSROW, valTDH, valTIC, valTICT, 
 valTIK, valTIKT, valTIM, valTIMT, valTIP, valTIPT, valTIW, valTIWT, valTIX, 
 valTIXT, valTPRCTS, valTPRODN, valttdh0, valttdh1, valttic, valttik, valttim, 
 valttip, valttiw, valttix, valVA, valW, valWC, valWTI, valXS, valXS_I, valYDH, 
 valYG, valYH, valYHK, valYHL, valYROW, valEV, valLEON, valEE, valNE, valEH, 
 valTFC_com, valTFC_ind, valTFC, valCO2I, valCO2NE, valCO2I2, valCO2NE2, 
 valTCO22, valTCO2I, valTCO2NE, valTCO2, valKOR_TFC
;