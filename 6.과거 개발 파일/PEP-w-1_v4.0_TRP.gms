...
$GDXIN DATA_AGG-2019_TRP_230111_MNG_NEW.gdx
$LOAD J, I1, BUS, PUB, F, L, K, ENE, Z, ZR, Z1, Zrich, Zother

 flow flow
 product energy carrier
 p_coal(product)
 p_coalprimary(product)
 p_coalproduct(product)
 p_ngas(product)
 p_oilproduct(product)
 f_telec(flow)
 f_tironstl(flow)
 f_agri(flow)
 f_mining(flow)
...
$GDXIN DATA_WEB_2019.gdx
$LOAD flow, product, p_coalprimary, p_coalproduct, p_ngas, p_oilproduct
$LOAD f_telec, f_tironstl
$LOAD f_agri, f_mining, f_foodpro, f_textiles,f_woodpaper, f_chemical, f_nonmet, f_ironstl
$LOAD f_nonferr, f_machine, f_transeq, f_otherind, f_construc, f_serpub, f_household
$LOAD f_roadrail, f_domeair, f_domewater
$LOAD f_nemining, f_nefoodpro, f_netextiles,f_newoodpaper, f_nechemical
$LOAD f_nenonmet, f_neironstl, f_nenonferr, f_nemachine, f_netranseq, f_neotherind
$LOAD f_neconstruc, f_netransport, f_neother
...
*$GDXIN DATA_AGG-2019.gdx
*$GDXIN DATA_AGG-2019_TRP_230111.gdx
*$GDXIN DATA_AGG-2019_TRP_230111_MNG.gdx
$GDXIN DATA_AGG-2019_TRP_230111_MNG_NEW.gdx

$LOAD CO, CGO, DDO, DEPO,  DIO, DSO, EXO, IMO, INVO, KSTO, LDO, MRGNO, XSO, RKDO,
$LOAD POPO, TDHO, TICO, TIKO, TIMO, TIPO, TIWO, TIXO, tmrg, sigma_M1, sigma_M2,
$LOAD sigma_VA
...
PARAMETER
PARZ;
*$CALL GDXXRW.EXE VAL_PAR.xls squeeze = 'no' par=sigma_KD rng=PAR!A5:R27 par=sigma_LD rng=PAR!A30:R52 par=sigma_X1 rng=PAR!A55:R77 par=sigma_X2 rng=PAR!A80:R102 par=sigma_y rng=PAR!A105:R127 par=PARZ rng=PAR!A129:C146 par=sigma_inv rng=PAR!A149:R171 Rdim=2 Cdim=1

$CALL GDXXRW.EXE VAL_TRP_PAR.xls squeeze = 'no' par=sigma_KD rng=PAR!A5:R29 par=sigma_LD rng=PAR!A32:R56 par=sigma_X1 rng=PAR!A59:R83 par=sigma_X2 rng=PAR!A86:R110 par=sigma_y rng=PAR!A113:R137 par=PARZ rng=PAR!A139:C156 par=sigma_inv rng=PAR!A159:R183 Rdim=2 Cdim=1

$GDXIN VAL_TRP_PAR.gdx
$LOAD sigma_KD, sigma_LD, sigma_X1, sigma_X2, sigma_y, PARZ
...
Display TDHO, TPRCTSO, TPRODNO, SHO, SGO, SROWO, ITO ;
*$exit
...
*Display  TIMO, ttimO, PWMO;
*execute_unload 'RUN_GAMS', PWMO, tmrg ;
...
*==============================================================================
*  4.3.5 Calibration of other prices and revised volumes (part 5)
*==============================================================================
Parameter
*Coal
elec_te_coal(f_telec, p_coalprimary),
ironstl_te_coal(f_tironstl, p_coalprimary),
agricult_fe_coal(f_agri, p_coalprimary),
mining_fe_coal(f_mining, p_coalprimary),
foodpro_fe_coal(f_foodpro, p_coalprimary),
textiles_fe_coal(f_textiles, p_coalprimary),
woodpaper_fe_coal(f_woodpaper, p_coalprimary),
chemical_fe_coal(f_chemical, p_coalprimary),
nonmet_fe_coal(f_nonmet, p_coalprimary),
ironstl_fe_coal(f_ironstl, p_coalprimary),
nonferr_fe_coal(f_nonferr, p_coalprimary),
machine_fe_coal(f_machine, p_coalprimary),
transeq_fe_coal(f_transeq, p_coalprimary),
otherind_fe_coal(f_otherind, p_coalprimary),
construc_fe_coal(f_construc, p_coalprimary),
roadrail_fe_coal(f_roadrail, p_coalprimary),
domewater_fe_coal(f_domewater, p_coalprimary)
domeair_fe_coal(f_domeair, p_coalprimary),
serpub_fe_coal(f_serpub, p_coalprimary),
household_fe_coal(f_household, p_coalprimary),

*Ngas
elec_te_ngas(f_telec, p_ngas),
agricult_fe_ngas(f_agri, p_ngas),
mining_fe_ngas(f_mining, p_ngas),
foodpro_fe_ngas(f_foodpro, p_ngas),
textiles_fe_ngas(f_textiles, p_ngas),
woodpaper_fe_ngas(f_woodpaper, p_ngas),
chemical_fe_ngas(f_chemical, p_ngas),
nonmet_fe_ngas(f_nonmet, p_ngas),
ironstl_fe_ngas(f_ironstl, p_ngas),
nonferr_fe_ngas(f_nonferr, p_ngas),
machine_fe_ngas(f_machine, p_ngas),
transeq_fe_ngas(f_transeq, p_ngas),
otherind_fe_ngas(f_otherind, p_ngas),
construc_fe_ngas(f_construc, p_ngas),
roadrail_fe_ngas(f_roadrail, p_ngas),
domewater_fe_ngas(f_domewater, p_ngas)
domeair_fe_ngas(f_domeair, p_ngas),
serpub_fe_ngas(f_serpub, p_ngas),
household_fe_ngas(f_household, p_ngas),

*Oilproduct
agricult_fe_oilproduct(f_agri, p_oilproduct),
mining_fe_oilproduct(f_mining, p_oilproduct),
foodpro_fe_oilproduct(f_foodpro, p_oilproduct),
textiles_fe_oilproduct(f_textiles, p_oilproduct),
woodpaper_fe_oilproduct(f_woodpaper, p_oilproduct),
chemical_fe_oilproduct(f_chemical, p_oilproduct),
nonmet_fe_oilproduct(f_nonmet, p_oilproduct),
ironstl_fe_oilproduct(f_ironstl, p_oilproduct),
nonferr_fe_oilproduct(f_nonferr, p_oilproduct),
machine_fe_oilproduct(f_machine, p_oilproduct),
transeq_fe_oilproduct(f_transeq, p_oilproduct),
otherind_fe_oilproduct(f_otherind, p_oilproduct),
construc_fe_oilproduct(f_construc, p_oilproduct),
serpub_fe_oilproduct(f_serpub, p_oilproduct),
household_fe_oilproduct(f_household, p_oilproduct),
roadrail_fe_oilproduct(f_roadrail, p_oilproduct),
domeair_fe_oilproduct(f_domeair, p_oilproduct),
domewater_fe_oilproduct(f_domewater, p_oilproduct),

mining_fne_oilproduct(f_nemining, p_oilproduct),
foodpro_fne_oilproduct(f_nefoodpro, p_oilproduct),
textiles_fne_oilproduct(f_netextiles, p_oilproduct),
woodpaper_fne_oilproduct(f_newoodpaper, p_oilproduct),
chemical_fne_oilproduct(f_nechemical, p_oilproduct),
nonmet_fne_oilproduct(f_nenonmet, p_oilproduct),
ironstl_fne_oilproduct(f_neironstl, p_oilproduct),
nonferr_fne_oilproduct(f_nenonferr, p_oilproduct),
machine_fne_oilproduct(f_nemachine, p_oilproduct),
transeq_fne_oilproduct(f_netranseq, p_oilproduct),
otherind_fne_oilproduct(f_neotherind, p_oilproduct),
construc_fne_oilproduct(f_neconstruc, p_oilproduct),
transport_fne_oilproduct(f_netransport, p_oilproduct),
other_fne_oilproduct(f_neother, p_oilproduct),

ecoeff_ind(ene,j,z) toe per 1000$ in Industry,
ecoeff_hoh(ene,z) toe per 1000$ in Household

necoeff_ind(ene,j,z) toe per 1000$ in Industry,
necoeff_hoh(ene,z) toe per 1000$ in Household
...
$GDXIN DATA_WEB_2019.gdx
*Coal
$LOAD elec_te_coal, ironstl_te_coal, agricult_fe_coal, mining_fe_coal, foodpro_fe_coal, textiles_fe_coal, woodpaper_fe_coal
$LOAD chemical_fe_coal, nonmet_fe_coal, ironstl_fe_coal, nonferr_fe_coal, machine_fe_coal, transeq_fe_coal, otherind_fe_coal
$LOAD construc_fe_coal, serpub_fe_coal, household_fe_coal, roadrail_fe_coal, domeair_fe_coal, domewater_fe_coal
*Ngas
$LOAD elec_te_ngas, agricult_fe_ngas, mining_fe_ngas, foodpro_fe_ngas, textiles_fe_ngas, woodpaper_fe_ngas
$LOAD chemical_fe_ngas, nonmet_fe_ngas, ironstl_fe_ngas, nonferr_fe_ngas, machine_fe_ngas, transeq_fe_ngas, otherind_fe_ngas
$LOAD construc_fe_ngas, serpub_fe_ngas, household_fe_ngas, roadrail_fe_ngas, domeair_fe_ngas, domewater_fe_ngas
*Oilproduct
$LOAD agricult_fe_oilproduct, mining_fe_oilproduct, foodpro_fe_oilproduct, textiles_fe_oilproduct, woodpaper_fe_oilproduct
$LOAD chemical_fe_oilproduct, nonmet_fe_oilproduct, ironstl_fe_oilproduct, nonferr_fe_oilproduct, machine_fe_oilproduct
$LOAD transeq_fe_oilproduct, otherind_fe_oilproduct, construc_fe_oilproduct, serpub_fe_oilproduct, household_fe_oilproduct
$LOAD roadrail_fe_oilproduct, domeair_fe_oilproduct, domewater_fe_oilproduct
*Oilproduct NE
$LOAD mining_fne_oilproduct, foodpro_fne_oilproduct, textiles_fne_oilproduct, woodpaper_fne_oilproduct, chemical_fne_oilproduct
$LOAD nonmet_fne_oilproduct, ironstl_fne_oilproduct, nonferr_fne_oilproduct, machine_fne_oilproduct, transeq_fne_oilproduct
$LOAD otherind_fne_oilproduct, construc_fne_oilproduct, transport_fne_oilproduct, other_fne_oilproduct

*===============================================================================
* Coal
*===============================================================================
ecoeff_ind('02_COAL','17_ELEC','01_KOR') = -1*[(sum((f_telec, p_coalprimary), elec_te_coal(f_telec, p_coalprimary))/RES) / DIO('02_COAL','17_ELEC','01_KOR')] ;
ecoeff_ind('02_COAL','12_IRONSTL ','01_KOR') =
{[-1*(sum((f_tironstl, p_coalprimary), ironstl_te_coal(f_tironstl, p_coalprimary))/RES)]+ sum((f_ironstl, p_coalprimary),ironstl_fe_coal(f_ironstl, p_coalprimary))/RES}
/ DIO('02_COAL','12_IRONSTL','01_KOR') ;

ecoeff_ind('02_COAL','01_AGRICULT','01_KOR') = (sum((f_agri, p_coalprimary), agricult_fe_coal(f_agri, p_coalprimary))/RES)/ DIO('02_COAL','01_AGRICULT','01_KOR') ;
ecoeff_ind('02_COAL','05_MINING','01_KOR') = (sum((f_mining, p_coalprimary), mining_fe_coal(f_mining, p_coalprimary))/RES)/ DIO('02_COAL','05_MINING','01_KOR') ;
ecoeff_ind('02_COAL','06_FOODPRO','01_KOR') = (sum((f_foodpro, p_coalprimary), foodpro_fe_coal(f_foodpro, p_coalprimary))/RES)/ DIO('02_COAL','06_FOODPRO','01_KOR') ;
ecoeff_ind('02_COAL','07_TEXTILES','01_KOR') = (sum((f_textiles, p_coalprimary), textiles_fe_coal(f_textiles, p_coalprimary))/RES)/ DIO('02_COAL','07_TEXTILES','01_KOR') ;
ecoeff_ind('02_COAL','08_WOODPRO','01_KOR') = (sum((f_woodpaper, p_coalprimary), woodpaper_fe_coal(f_woodpaper, p_coalprimary))/RES)/ DIO('02_COAL','08_WOODPRO','01_KOR') ;
ecoeff_ind('02_COAL','10_CHEMICAL','01_KOR') = (sum((f_chemical, p_coalprimary), chemical_fe_coal(f_chemical, p_coalprimary))/RES)/ DIO('02_COAL','10_CHEMICAL','01_KOR') ;
ecoeff_ind('02_COAL','11_NONMET','01_KOR') = (sum((f_nonmet, p_coalprimary), nonmet_fe_coal(f_nonmet, p_coalprimary))/RES)/ DIO('02_COAL','11_NONMET','01_KOR') ;
ecoeff_ind('02_COAL','13_NONFERR','01_KOR') = (sum((f_nonferr, p_coalprimary), nonferr_fe_coal(f_nonferr, p_coalprimary))/RES)/ DIO('02_COAL','13_NONFERR','01_KOR') ;
ecoeff_ind('02_COAL','14_MACHINE','01_KOR') = (sum((f_machine, p_coalprimary), machine_fe_coal(f_machine, p_coalprimary))/RES)/ DIO('02_COAL','14_MACHINE','01_KOR') ;
ecoeff_ind('02_COAL','15_TRANSEQ','01_KOR') = (sum((f_transeq, p_coalprimary), transeq_fe_coal(f_transeq, p_coalprimary))/RES)/ DIO('02_COAL','15_TRANSEQ','01_KOR') ;
ecoeff_ind('02_COAL','16_OTHERIND','01_KOR') = (sum((f_otherind, p_coalprimary), otherind_fe_coal(f_otherind, p_coalprimary))/RES)/ DIO('02_COAL','16_OTHERIND','01_KOR') ;
ecoeff_ind('02_COAL','19_CONSTRUC','01_KOR') = (sum((f_construc, p_coalprimary), construc_fe_coal(f_construc, p_coalprimary))/RES)/ DIO('02_COAL','19_CONSTRUC','01_KOR') ;
ecoeff_ind('02_COAL','20_LTRP','01_KOR') = (sum((f_roadrail, p_coalprimary), roadrail_fe_coal(f_roadrail, p_coalprimary))/RES)/ DIO('02_COAL','20_LTRP','01_KOR') ;
ecoeff_ind('02_COAL','21_WTRP','01_KOR') = (sum((f_domewater, p_coalprimary), domewater_fe_coal(f_domewater, p_coalprimary))/RES)/ DIO('02_COAL','21_WTRP','01_KOR') ;
ecoeff_ind('02_COAL','22_ATRP','01_KOR') = (sum((f_domeair, p_coalprimary), domeair_fe_coal(f_domeair, p_coalprimary))/RES)/ DIO('02_COAL','22_ATRP','01_KOR') ;

ecoeff_ind('02_COAL','23_SER','01_KOR') = (sum((f_serpub, p_coalprimary), serpub_fe_coal(f_serpub, p_coalprimary))/RES)*[DIO('02_COAL','23_SER','01_KOR')/(DIO('02_COAL','23_SER','01_KOR')+DIO('02_COAL','24_PUB','01_KOR'))]/ DIO('02_COAL','23_SER','01_KOR') ;
ecoeff_ind('02_COAL','24_PUB','01_KOR') = (sum((f_serpub, p_coalprimary), serpub_fe_coal(f_serpub, p_coalprimary))/RES)*[DIO('02_COAL','24_PUB','01_KOR')/(DIO('02_COAL','23_SER','01_KOR')+DIO('02_COAL','24_PUB','01_KOR'))]/ DIO('02_COAL','24_PUB','01_KOR') ;

ecoeff_hoh('02_COAL','01_KOR') = (sum((f_household, p_coalprimary), household_fe_coal(f_household, p_coalprimary))/RES)/ CO('02_COAL', '01_KOR') ;

*===============================================================================
* Ngas
*===============================================================================
ecoeff_ind('04_NGAS','17_ELEC','01_KOR') = -1*[(sum((f_telec, p_ngas), elec_te_ngas(f_telec, p_ngas))/RES) / DIO('04_NGAS','17_ELEC','01_KOR')] ;

ecoeff_ind('04_NGAS','01_AGRICULT','01_KOR') = (sum((f_agri, p_ngas), agricult_fe_ngas(f_agri, p_ngas))/RES)/ DIO('04_NGAS','01_AGRICULT','01_KOR') ;
ecoeff_ind('04_NGAS','05_MINING','01_KOR') = (sum((f_mining, p_ngas), mining_fe_ngas(f_mining, p_ngas))/RES)/ DIO('04_NGAS','05_MINING','01_KOR') ;
ecoeff_ind('04_NGAS','06_FOODPRO','01_KOR') = (sum((f_foodpro, p_ngas), foodpro_fe_ngas(f_foodpro, p_ngas))/RES)/ DIO('04_NGAS','06_FOODPRO','01_KOR') ;
ecoeff_ind('04_NGAS','07_TEXTILES','01_KOR') = (sum((f_textiles, p_ngas), textiles_fe_ngas(f_textiles, p_ngas))/RES)/ DIO('04_NGAS','07_TEXTILES','01_KOR') ;
ecoeff_ind('04_NGAS','08_WOODPRO','01_KOR') = (sum((f_woodpaper, p_ngas), woodpaper_fe_ngas(f_woodpaper, p_ngas))/RES)/ DIO('04_NGAS','08_WOODPRO','01_KOR') ;
ecoeff_ind('04_NGAS','10_CHEMICAL','01_KOR') = (sum((f_chemical, p_ngas), chemical_fe_ngas(f_chemical, p_ngas))/RES)/ DIO('04_NGAS','10_CHEMICAL','01_KOR') ;
ecoeff_ind('04_NGAS','11_NONMET','01_KOR') = (sum((f_nonmet, p_ngas), nonmet_fe_ngas(f_nonmet, p_ngas))/RES)/ DIO('04_NGAS','11_NONMET','01_KOR') ;
ecoeff_ind('04_NGAS','13_NONFERR','01_KOR') = (sum((f_nonferr, p_ngas), nonferr_fe_ngas(f_nonferr, p_ngas))/RES)/ DIO('04_NGAS','13_NONFERR','01_KOR') ;
ecoeff_ind('04_NGAS','14_MACHINE','01_KOR') = (sum((f_machine, p_ngas), machine_fe_ngas(f_machine, p_ngas))/RES)/ DIO('04_NGAS','14_MACHINE','01_KOR') ;
ecoeff_ind('04_NGAS','15_TRANSEQ','01_KOR') = (sum((f_transeq, p_ngas), transeq_fe_ngas(f_transeq, p_ngas))/RES)/ DIO('04_NGAS','15_TRANSEQ','01_KOR') ;
ecoeff_ind('04_NGAS','16_OTHERIND','01_KOR') = (sum((f_otherind, p_ngas), otherind_fe_ngas(f_otherind, p_ngas))/RES)/ DIO('04_NGAS','16_OTHERIND','01_KOR') ;
ecoeff_ind('04_NGAS','19_CONSTRUC','01_KOR') = (sum((f_construc, p_ngas), construc_fe_ngas(f_construc, p_ngas))/RES)/ DIO('04_NGAS','19_CONSTRUC','01_KOR') ;
ecoeff_ind('04_NGAS','20_LTRP','01_KOR') = (sum((f_roadrail, p_ngas), roadrail_fe_ngas(f_roadrail, p_ngas))/RES)/ DIO('04_NGAS','20_LTRP','01_KOR') ;
ecoeff_ind('04_NGAS','21_WTRP','01_KOR') = (sum((f_domewater, p_ngas), domewater_fe_ngas(f_domewater, p_ngas))/RES)/ DIO('04_NGAS','21_WTRP','01_KOR') ;
ecoeff_ind('04_NGAS','22_ATRP','01_KOR') = (sum((f_domeair, p_ngas), domeair_fe_ngas(f_domeair, p_ngas))/RES)/ DIO('04_NGAS','22_ATRP','01_KOR') ;

ecoeff_ind('04_NGAS','23_SER','01_KOR') = (sum((f_serpub, p_ngas), serpub_fe_ngas(f_serpub, p_ngas))/RES)*[DIO('04_NGAS','23_SER','01_KOR')/(DIO('04_NGAS','23_SER','01_KOR')+DIO('04_NGAS','24_PUB','01_KOR'))]/ DIO('04_NGAS','23_SER','01_KOR') ;
ecoeff_ind('04_NGAS','24_PUB','01_KOR') = (sum((f_serpub, p_ngas), serpub_fe_ngas(f_serpub, p_ngas))/RES)*[DIO('04_NGAS','24_PUB','01_KOR')/(DIO('04_NGAS','23_SER','01_KOR')+DIO('04_NGAS','24_PUB','01_KOR'))]/ DIO('04_NGAS','24_PUB','01_KOR') ;

ecoeff_hoh('04_NGAS','01_KOR') = (sum((f_household, p_ngas), household_fe_ngas(f_household, p_ngas))/RES)/ CO('04_NGAS', '01_KOR') ;

*===============================================================================
* Oilproduct
*===============================================================================
ecoeff_ind('09_PETROLCOAL','01_AGRICULT','01_KOR') = (sum((f_agri, p_oilproduct), agricult_fe_oilproduct(f_agri, p_oilproduct))/RES)/ DIO('09_PETROLCOAL','01_AGRICULT','01_KOR') ;
ecoeff_ind('09_PETROLCOAL','05_MINING','01_KOR') = (sum((f_mining, p_oilproduct), mining_fe_oilproduct(f_mining, p_oilproduct))/RES)/ DIO('09_PETROLCOAL','05_MINING','01_KOR') ;
ecoeff_ind('09_PETROLCOAL','06_FOODPRO','01_KOR') = (sum((f_foodpro, p_oilproduct), foodpro_fe_oilproduct(f_foodpro, p_oilproduct))/RES)/ DIO('09_PETROLCOAL','06_FOODPRO','01_KOR') ;
ecoeff_ind('09_PETROLCOAL','07_TEXTILES','01_KOR') = (sum((f_textiles, p_oilproduct), textiles_fe_oilproduct(f_textiles, p_oilproduct))/RES)/ DIO('09_PETROLCOAL','07_TEXTILES','01_KOR') ;
ecoeff_ind('09_PETROLCOAL','08_WOODPRO','01_KOR') = (sum((f_woodpaper, p_oilproduct), woodpaper_fe_oilproduct(f_woodpaper, p_oilproduct))/RES)/ DIO('09_PETROLCOAL','08_WOODPRO','01_KOR') ;
ecoeff_ind('09_PETROLCOAL','10_CHEMICAL','01_KOR') = (sum((f_chemical, p_oilproduct), chemical_fe_oilproduct(f_chemical, p_oilproduct))/RES)/ DIO('09_PETROLCOAL','10_CHEMICAL','01_KOR') ;
ecoeff_ind('09_PETROLCOAL','11_NONMET','01_KOR') = (sum((f_nonmet, p_oilproduct), nonmet_fe_oilproduct(f_nonmet, p_oilproduct))/RES)/ DIO('09_PETROLCOAL','11_NONMET','01_KOR') ;
ecoeff_ind('09_PETROLCOAL','13_NONFERR','01_KOR') = (sum((f_nonferr, p_oilproduct), nonferr_fe_oilproduct(f_nonferr, p_oilproduct))/RES)/ DIO('09_PETROLCOAL','13_NONFERR','01_KOR') ;
ecoeff_ind('09_PETROLCOAL','14_MACHINE','01_KOR') = (sum((f_machine, p_oilproduct), machine_fe_oilproduct(f_machine, p_oilproduct))/RES)/ DIO('09_PETROLCOAL','14_MACHINE','01_KOR') ;
ecoeff_ind('09_PETROLCOAL','15_TRANSEQ','01_KOR') = (sum((f_transeq, p_oilproduct), transeq_fe_oilproduct(f_transeq, p_oilproduct))/RES)/ DIO('09_PETROLCOAL','15_TRANSEQ','01_KOR') ;
ecoeff_ind('09_PETROLCOAL','16_OTHERIND','01_KOR') = (sum((f_otherind, p_oilproduct), otherind_fe_oilproduct(f_otherind, p_oilproduct))/RES)/ DIO('09_PETROLCOAL','16_OTHERIND','01_KOR') ;
ecoeff_ind('09_PETROLCOAL','19_CONSTRUC','01_KOR') = (sum((f_construc, p_oilproduct), construc_fe_oilproduct(f_construc, p_oilproduct))/RES)/ DIO('09_PETROLCOAL','19_CONSTRUC','01_KOR') ;
ecoeff_ind('09_PETROLCOAL','20_LTRP','01_KOR') = (sum((f_roadrail, p_oilproduct), roadrail_fe_oilproduct(f_roadrail, p_oilproduct))/RES)/ DIO('09_PETROLCOAL','20_LTRP','01_KOR') ;
ecoeff_ind('09_PETROLCOAL','21_WTRP','01_KOR') = (sum((f_domewater, p_oilproduct), domewater_fe_oilproduct(f_domewater, p_oilproduct))/RES)/ DIO('09_PETROLCOAL','21_WTRP','01_KOR') ;
ecoeff_ind('09_PETROLCOAL','22_ATRP','01_KOR') = (sum((f_domeair, p_oilproduct), domeair_fe_oilproduct(f_domeair, p_oilproduct))/RES)/ DIO('09_PETROLCOAL','22_ATRP','01_KOR') ;

ecoeff_ind('09_PETROLCOAL','23_SER','01_KOR') = (sum((f_serpub, p_oilproduct), serpub_fe_oilproduct(f_serpub, p_oilproduct))/RES)*DIO('09_PETROLCOAL','23_SER','01_KOR')/(DIO('09_PETROLCOAL','23_SER','01_KOR')+DIO('09_PETROLCOAL','24_PUB','01_KOR'))/ DIO('09_PETROLCOAL','23_SER','01_KOR') ;
ecoeff_ind('09_PETROLCOAL','24_PUB','01_KOR') = (sum((f_serpub, p_oilproduct), serpub_fe_oilproduct(f_serpub, p_oilproduct))/RES)*DIO('09_PETROLCOAL','24_PUB','01_KOR')/(DIO('09_PETROLCOAL','23_SER','01_KOR')+DIO('09_PETROLCOAL','24_PUB','01_KOR'))/ DIO('09_PETROLCOAL','24_PUB','01_KOR') ;

ecoeff_hoh('09_PETROLCOAL','01_KOR') = (sum((f_household, p_oilproduct), household_fe_oilproduct(f_household, p_oilproduct))/RES)/ CO('09_PETROLCOAL', '01_KOR') ;

necoeff_ind('09_PETROLCOAL','05_MINING','01_KOR') = (sum((f_nemining, p_oilproduct), mining_fne_oilproduct(f_nemining, p_oilproduct))/RES)/ DIO('09_PETROLCOAL','05_MINING','01_KOR') ;
necoeff_ind('09_PETROLCOAL','06_FOODPRO','01_KOR') = (sum((f_nefoodpro, p_oilproduct), foodpro_fne_oilproduct(f_nefoodpro, p_oilproduct))/RES)/ DIO('09_PETROLCOAL','06_FOODPRO','01_KOR') ;
necoeff_ind('09_PETROLCOAL','07_TEXTILES','01_KOR') = (sum((f_netextiles, p_oilproduct), textiles_fne_oilproduct(f_netextiles, p_oilproduct))/RES)/ DIO('09_PETROLCOAL','07_TEXTILES','01_KOR') ;
necoeff_ind('09_PETROLCOAL','08_WOODPRO','01_KOR') = (sum((f_newoodpaper, p_oilproduct), woodpaper_fne_oilproduct(f_newoodpaper, p_oilproduct))/RES)/ DIO('09_PETROLCOAL','08_WOODPRO','01_KOR') ;
necoeff_ind('09_PETROLCOAL','10_CHEMICAL','01_KOR') = (sum((f_nechemical, p_oilproduct), chemical_fne_oilproduct(f_nechemical, p_oilproduct))/RES)/ DIO('09_PETROLCOAL','10_CHEMICAL','01_KOR') ;
necoeff_ind('09_PETROLCOAL','11_NONMET','01_KOR') = (sum((f_nenonmet, p_oilproduct), nonmet_fne_oilproduct(f_nenonmet, p_oilproduct))/RES)/ DIO('09_PETROLCOAL','11_NONMET','01_KOR') ;
necoeff_ind('09_PETROLCOAL','13_NONFERR','01_KOR') = (sum((f_nenonferr, p_oilproduct), nonferr_fne_oilproduct(f_nenonferr, p_oilproduct))/RES)/ DIO('09_PETROLCOAL','13_NONFERR','01_KOR') ;
necoeff_ind('09_PETROLCOAL','14_MACHINE','01_KOR') = (sum((f_nemachine, p_oilproduct), machine_fne_oilproduct(f_nemachine, p_oilproduct))/RES)/ DIO('09_PETROLCOAL','14_MACHINE','01_KOR') ;
necoeff_ind('09_PETROLCOAL','15_TRANSEQ','01_KOR') = (sum((f_netranseq, p_oilproduct), transeq_fne_oilproduct(f_netranseq, p_oilproduct))/RES)/ DIO('09_PETROLCOAL','15_TRANSEQ','01_KOR') ;
necoeff_ind('09_PETROLCOAL','16_OTHERIND','01_KOR') = (sum((f_neotherind, p_oilproduct), otherind_fne_oilproduct(f_neotherind, p_oilproduct))/RES)/ DIO('09_PETROLCOAL','16_OTHERIND','01_KOR') ;
necoeff_ind('09_PETROLCOAL','19_CONSTRUC','01_KOR') = (sum((f_neconstruc, p_oilproduct), construc_fne_oilproduct(f_neconstruc, p_oilproduct))/RES)/ DIO('09_PETROLCOAL','19_CONSTRUC','01_KOR') ;
necoeff_ind('09_PETROLCOAL','20_LTRP','01_KOR') = [(sum((f_netransport, p_oilproduct), transport_fne_oilproduct(f_netransport, p_oilproduct))/RES)*[(DIO('09_PETROLCOAL','20_LTRP','01_KOR'))/(DIO('09_PETROLCOAL','20_LTRP','01_KOR')+DIO('09_PETROLCOAL','21_WTRP','01_KOR')+DIO('09_PETROLCOAL','22_ATRP','01_KOR'))]]/ DIO('09_PETROLCOAL','20_LTRP','01_KOR') ;
necoeff_ind('09_PETROLCOAL','21_WTRP','01_KOR') = [(sum((f_netransport, p_oilproduct), transport_fne_oilproduct(f_netransport, p_oilproduct))/RES)*[(DIO('09_PETROLCOAL','21_WTRP','01_KOR'))/(DIO('09_PETROLCOAL','20_LTRP','01_KOR')+DIO('09_PETROLCOAL','21_WTRP','01_KOR')+DIO('09_PETROLCOAL','22_ATRP','01_KOR'))]]/ DIO('09_PETROLCOAL','21_WTRP','01_KOR') ;
necoeff_ind('09_PETROLCOAL','22_ATRP','01_KOR') = [(sum((f_netransport, p_oilproduct), transport_fne_oilproduct(f_netransport, p_oilproduct))/RES)*[(DIO('09_PETROLCOAL','22_ATRP','01_KOR'))/(DIO('09_PETROLCOAL','20_LTRP','01_KOR')+DIO('09_PETROLCOAL','21_WTRP','01_KOR')+DIO('09_PETROLCOAL','22_ATRP','01_KOR'))]]/ DIO('09_PETROLCOAL','22_ATRP','01_KOR') ;

necoeff_ind('09_PETROLCOAL','01_AGRICULT','01_KOR') = [(sum((f_neother, p_oilproduct), other_fne_oilproduct(f_neother, p_oilproduct))/RES)*[DIO('09_PETROLCOAL','01_AGRICULT','01_KOR')/(DIO('09_PETROLCOAL','01_AGRICULT','01_KOR')+DIO('09_PETROLCOAL','23_SER','01_KOR')+DIO('09_PETROLCOAL','24_PUB','01_KOR')+CO('09_PETROLCOAL', '01_KOR'))] ]/ DIO('09_PETROLCOAL','01_AGRICULT','01_KOR') ;
necoeff_ind('09_PETROLCOAL','23_SER','01_KOR') = [(sum((f_neother, p_oilproduct), other_fne_oilproduct(f_neother, p_oilproduct))/RES)*[DIO('09_PETROLCOAL','23_SER','01_KOR')/(DIO('09_PETROLCOAL','01_AGRICULT','01_KOR')+DIO('09_PETROLCOAL','23_SER','01_KOR')+DIO('09_PETROLCOAL','24_PUB','01_KOR')+CO('09_PETROLCOAL', '01_KOR'))] ]/ DIO('09_PETROLCOAL','23_SER','01_KOR') ;
necoeff_ind('09_PETROLCOAL','24_PUB','01_KOR') = [(sum((f_neother, p_oilproduct), other_fne_oilproduct(f_neother, p_oilproduct))/RES)*[DIO('09_PETROLCOAL','24_PUB','01_KOR')/(DIO('09_PETROLCOAL','01_AGRICULT','01_KOR')+DIO('09_PETROLCOAL','23_SER','01_KOR')+DIO('09_PETROLCOAL','24_PUB','01_KOR')+CO('09_PETROLCOAL', '01_KOR'))] ]/ DIO('09_PETROLCOAL','24_PUB','01_KOR') ;
necoeff_hoh('09_PETROLCOAL','01_KOR') = [(sum((f_neother, p_oilproduct), other_fne_oilproduct(f_neother, p_oilproduct))/RES)*[CO('09_PETROLCOAL', '01_KOR')/(DIO('09_PETROLCOAL','01_AGRICULT','01_KOR')+DIO('09_PETROLCOAL','23_SER','01_KOR')+DIO('09_PETROLCOAL','24_PUB','01_KOR')+CO('09_PETROLCOAL', '01_KOR'))] ]/ CO('09_PETROLCOAL', '01_KOR') ;
...
* 23_SER          Service
* 24_PUB          Public Service

display ecoeff_ind, ecoeff_hoh, necoeff_ind, necoeff_hoh ;
*$exit
...
*==============================================================================
 ...
 beta_EX_X1(j,z)$MRGNO(j,z)
                 = [PETO(j,z)*EXTO(j,z)$EXTO(j,z)**(1-rho_X1(j,z))]/
                   ...
                   eO(z)*PWMGO(j)*MRGNO(j,z)$MRGNO(j,z)**(1-rho_X1(j,z))+Tiny];

display PETO, EXO, EXTO, rho_X1, DSO, PLO, rho_X1 ;

 beta_EX_X1(j,z)$(MRGNO(j,z) eq 0)
                 = [PETO(j,z)*EXTO(j,z)$EXTO(j,z)**(1-rho_X1(j,z))]/
                   [PETO(j,z)*EXTO(j,z)$EXTO(j,z)**(1-rho_X1(j,z))+
                   PLO(j,z)*DSO(j,z)**(1-rho_X1(j,z))+Tiny];
...
display rho_KD, sigma_KD, KDO, LDO, RTIO, KDCO, LDCO;
...
display EXO, beta_X2, rho_X2, B_X2 ;
*$exit
...
*==============================================================================
 ...
 DS(i,z)         Supply of commodity i to the domestic market of region z
...
*==============================================================================
 ...
 TIP(i,z)        Government revenue from taxes on industry j production in region z (excluding taxes directly related to the use of capital and labor)
...
EQUATIONS
 ...
 EQ40(i,z)       CET function between exports local production and margins
...
*   5.3.1 Production
*==============================================================================
 EQ1(j,z)..      VA(j,z) =e= v(j,z)*XS(j,z);

 EQ2(j,z)..      CI(j,z) =e= io(j,z)*XS(j,z);
...
* EQ19(z)..       TIKT(z) =e= SUM[(k,j),TIK(k,j,z)];
...
*                 TIK(k,j,z) =e= ttik(k,j,z)*R(k,j,z)*KD(k,j,z);
 ...
 EQ28(j,z)..     TIP(j,z) =e= ttip(j,z)*PP(j,z)*XS(j,z);
...
*==============================================================================
 EQ40(i,z)..     XS(i,z) =e= B_X1(i,z)*{
        [beta_EX_X1(i,z)*EXT(i,z)**rho_X1(i,z)]$EXTO(i,z)
       +[beta_D_X1(i,z)*DS(i,z)**rho_X1(i,z)]$DSO(i,z)
       +[(1-beta_EX_X1(i,z)-beta_D_X1(i,z))*MRGN(i,z)**rho_X1(i,z)]$MRGNO(i,z)
                                          }**(1/rho_X1(i,z));

 EQ41(i,z)$[EXTO(i,z) and DSO(i,z)]..
                 EXT(i,z) =e= {[beta_D_X1(i,z)/beta_EX_X1(i,z)]*[PET(i,z)
                                /PL(i,z)]}**sigma_X1(i,z)*DS(i,z);

 EQ42(i,z)$[MRGNO(i,z) and DSO(i,z)]..
...
*   5.3.5 Prices
*==============================================================================
 EQ49(j,z)..     PP(j,z)*XS(j,z) =e= PVA(j,z)*VA(j,z)+PCI(j,z)*CI(j,z);

 EQ50(j,z)..     P(j,z) =e= (1+ttip(j,z))*PP(j,z);
...
*                 RC(j,z)*KDC(j,z) =e= SUM[k$KDO(k,j,z),RTI(k,j,z)*KD(k,j,z)];
 ...
 EQ58(j,z)..     P(j,z)*XS(j,z) =e= [PL(j,z)*DS(j,z)]$DSO(j,z)
...
*ttim.FX(i,zj,z) = 0;
*ttim.FX(i,zj,z) = ttimO(i,zj,z)*0.5;
