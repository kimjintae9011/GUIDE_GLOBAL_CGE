...
display sigma_Y, gamma_LES, CMINO;
...
* CO2FACTOR('04_GAS','17_OTHERIND',z) = 0;
...
execute_unload 'CO2FACTOR_woKLEM',
 CO2FACTOR, DEO, CO2IO, Coal_DIO, p_coal ;
...
PARAMETER
 ...
 valEH(product,z,sim)
 valETOT(product,z,sim)
 
 valEE_p(product,j,z,sim)
 valNE_p(product,j,z,sim) 
 valEH_p(product,z,sim)
 valETOT_p(product,z,sim)

 valCO2I(product,j,z,sim)
 valCO2NE(product,j,z,sim) 
 valCO2H(product,z,sim)   
 valCH4I(product,j,z,sim)
 valCH4H(product,z,sim)   
 valN2OI(product,j,z,sim) 
 valN2OH(product,z,sim)
 valCO2I2(j,z,sim)  
 valCO2NE2(j,z,sim) 
 valTCO2I(z,sim)
 valTCO2NE(z,sim)
...
*==============================================================================
 ...
 TCTAX(z)           Government revenuse from Carbon tax
...
*   5.3.5 Prices
*==============================================================================
 EQ49(j,z)..     PP(j,z)*XST(j,z) =e= PVA(j,z)*VA(j,z)+PCI(j,z)*CI(j,z)+PCE(j,z)*CE(j,z);
...
*                 P4(ene,j,z) =e= PC(ene,z)*(1+CTAX(Z)*CO2FACTOR(ene,j,z));
...
*============================================================================== 
*$ontext
 LD.LO(l,j,z)   = 0.00001*LD.l(l,j,z);
 LDC.LO(j,z)    = 0.0000001*LDC.l(j,z);
 WTI.LO(l,j,z)  = 0.0000001*WTI.l(l,j,z); 
 WC.LO(j,z)     = 0.0000001*WC.l(j,z);
 RC.LO(j,z)     = 0.0000001*RC.l(j,z);
 R.LO(k,j,z)    = 0.0000001*R.l(k,j,z);
 RTI.LO(k,j,z)  = 0.0000001*RTI.l(k,j,z);
 DS_I.LO(i,z)   = 0.00001*DS_I.l(i,z);
 EXT.LO(i,z)    = 0.00001*EXT.l(i,z);
 EX.LO(i,z,zj)  = 0.00001*EX.l(i,z,zj);
 IM.LO(i,zj,z)  = 0.00001*IM.l(i,zj,z);
 IMT.LO(i,z)    = 0.00001*IMT.l(i,z);
 KD.LO(k,j,z)   = 0.00001*KD.L(k,j,z);
 KDC.LO(j,z)    = 0.00001*KDC.L(j,z);
 VA.LO(j,z)     = 0.00001*VA.L(j,z);
 XS.LO(j,i,z)   = 0.00001*XS.L(j,i,z);
 XS_I.LO(i,z)   = 0.00001*XS_I.L(i,z);
 XST.LO(j,z)    = 0.00001*XST.L(j,z);
 PE.LO(i,z,zj)  = 0.00001*PE.L(i,z,zj);
 PET.LO(i,z)    = 0.00001*PET.L(i,z);
 PK.LO(z)       = 0.00001*PK.L(z);
 PL.LO(i,z)     = 0.00001*PL.L(i,z);
 PM.LO(i,zj,z)  = 0.00001*PM.L(i,zj,z);
 PMT.LO(i,z)    = 0.00001*PMT.L(i,z);
 PP.LO(j,z)     = 0.00001*PP.L(j,z);
 PVA.LO(j,z)    = 0.00001*PVA.L(j,z);
 PWM.LO(i,zj,z) = 0.00001*PWM.L(i,zj,z);
 PWMG.LO(i)     = 0.00001*PWMG.L(i);
 PWX.LO(i,z,zj) = 0.00001*PWX.L(i,z,zj);
 PCE.LO(j,z)    = 0.00001*PCE.l(j,z);
*$offtext
*==============================================================================
...
*  6.4 Simulation
*==============================================================================
*ttim.FX(ene,zj,z2) = ttimO(ENE,zj,z2)*10;
*ttim.FX(i,zj,z) = ttimO(i,zj,z)*0.8;
*ttic.FX('18_ELEC','01_KOR') = tticO('18_ELEC','01_KOR')*1.1;
...
*$exit
*$INCLUDE RESULTS_PEP-w-1_v4.0_231208_woKLEM.gms
...
SOLVE PEPW1 USING CNS;
 ...
 valETOT(product,z,sim)         = sum(j,valEE(product,j,z,sim)) + sum(j,valNE(product,j,z,sim)) + valEH(product,z,sim) ;

 valEE_p(product,j,z,sim)       = valEE(product,j,z,sim)*1000*41.868/physical_unit(product,'physical');
 valNE_p(product,j,z,sim)       = valNE(product,j,z,sim)*1000*41.868/physical_unit(product,'physical');
 valEH_p(product,z,sim)         = valEH(product,z,sim)*1000*41.868/physical_unit(product,'physical');
 valETOT_p(product,z,sim)       = sum(j,valEE_p(product,j,z,sim)) + sum(j,valNE_p(product,j,z,sim)) + valEH_p(product,z,sim) ;
 ...
 valTCO2(z,sim)                 = valTCO2H(z,sim) + valTCO2I(z,sim) +valTCO2NE(z,sim) ;
...
execute_unload 'results_PEP-w-1_v4.0_231208_woKLEM_CTAX',

 valEE, valNE, valEH,valETOT,
 valEE_p,valNE_p, valEH_p, valETOT_p,
 valCO2I, valCO2NE, valCO2H,
 valCH4I, valCH4H,
 valN2OI, valN2OH,
 valCO2I2, valCO2NE2, 
 valTCO2I, valTCO2NE, valTCO2H,
 valTCO2,valCTAX,
 valPOWER,
 valGDP_BP,valGDP_BP_REAL
 ;