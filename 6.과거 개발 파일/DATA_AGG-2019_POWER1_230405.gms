...
*==============================================================================
$CALL har2gdx basedata2019_230405_POWER1 basedata2019.gdx
$CALL har2gdx parameter2019_POWER1.prm Parameters2019.gdx
...
/
 ...
 22_eWSHo       Wind Solar Hydro Other generation
 23_eOil        Oil generation
 24_CONSTRUC    Construction
 25_LTRP        Land transport service(road rail)
 26_WTRP        Water transport service
 27_ATRP        Air transport service
 28_SER         Service
/
...
*==============================================================================
 ...
 marg(acts)  Margin commodities
 comm(acts)  All sectors and commodities except for investment
...
j2comm(j,acts) Mapping of sectors
/
 ...
 26_WTRP.         (wtp)
 27_ATRP.         (atp)
 28_SER.          (trd, afs, whs, cmn, ofi, ins, rsa, obs, ros, dwe, osg, edu, hht, wtr)
/
...
* agents prices (VDPP) and private household imports at agents prices (VMPP).
 ...
 CO(i,z)         = SUM{(comm,reg)$[j2comm(i,comm)$z2reg(z,reg)],
...
* agents prices (VDGP) and public final imports at agents prices (VMGP).
 ...
 CGO(i,z)        = SUM{(comm,reg)$[j2comm(i,comm)$z2reg(z,reg)],
...
*  and public final imports at agents prices (VIFA) for this sector.
 ...
 INVO(i,z)       = SUM{(comm,reg)$[j2comm(i,comm)$z2reg(z,reg)],
...
* agents prices (VDFP) and public final imports at agents prices (VIFA).
 ...
 DIO(i,j,z)      = SUM[(comm,commj,reg)$[j2comm(i,comm)
                      $j2comm(j,commj)$z2reg(z,reg)],
                      VDFP(comm,commj,reg)
                     +VMFP(comm,commj,reg)];

 tssm(i,j,z)    = SUM[(comm,commj,reg)$[j2comm(i,comm)
                      $j2comm(j,commj)$z2reg(z,reg)],
                      VMFP(comm,commj,reg)-VMFB(comm,commj,reg)];

 tssd(i,j,z)    = SUM[(comm,commj,reg)$[j2comm(i,comm)
                      $j2comm(j,commj)$z2reg(z,reg)],
                      VDFP(comm,commj,reg)-VDFB(comm,commj,reg)];
...
** is removed.
 ...
 IMO(i,zj,z)     = SUM{(comm,regj,reg)$[j2comm(i,comm)
                       $z2reg(zj,regj)$z2reg(z,reg)],
                       VCIF(comm,regj,reg)-
                       SUM[marg,VTMFSD(marg,comm,regj,reg)]};

 IMO('19_eNuclear', '03_JPN', z) = 0.00001;
 IMO('19_eNuclear','03_JPN','03_JPN') = 0;

 IMO('19_eNuclear', '05_MNG', z) = 0.00001;
 IMO('19_eNuclear','05_MNG','05_MNG') = 0;

 IMO('19_eNuclear', '06_PRK', z) = 0.00001;
 IMO('19_eNuclear','06_PRK','06_PRK') = 0;

 IMO('19_eNuclear', '14_CPA', z) = 0.00001;
 IMO('19_eNuclear','14_CPA','14_CPA') = 0;

 IMO('19_eNuclear', '17_PAO', z) = 0.00001;
 IMO('19_eNuclear','17_PAO','17_PAO') = 0;

* IMO('19_eNuclear','05_MNG','01_KOR') = 0.00000001;
* IMO('19_eNuclear','06_PRK','01_KOR') = 0.00000001;
* IMO('19_eNuclear','14_CPA','01_KOR') = 0.00000001;
* IMO('19_eNuclear','17_PAO','01_KOR') = 0.00000001;
...
* tmrg('20_LTRP',ij,zj,z)$IMO(ij,zj,z)
  tmrg('25_LTRP',ij,zj,z)$IMO(ij,zj,z)
                 = SUM{(comm,regj,reg)$[j2comm(ij,comm)
                       $z2reg(zj,regj)$z2reg(z,reg)],
                       VTMFSD('otp',comm,regj,reg)};

* tmrg('21_WTRP',ij,zj,z)$IMO(ij,zj,z)
  tmrg('26_WTRP',ij,zj,z)$IMO(ij,zj,z)
                 = SUM{(comm,regj,reg)$[j2comm(ij,comm)
                       $z2reg(zj,regj)$z2reg(z,reg)],
                       VTMFSD('wtp',comm,regj,reg)};

* tmrg('22_ATRP',ij,zj,z)$IMO(ij,zj,z)
 tmrg('27_ATRP',ij,zj,z)$IMO(ij,zj,z)
                 = SUM{(comm,regj,reg)$[j2comm(ij,comm)
                       $z2reg(zj,regj)$z2reg(z,reg)],
                       VTMFSD('atp',comm,regj,reg)};


 tmrg3(ij,zj) = tmrg('25_LTRP',ij,zj,'01_KOR') ;

 tmrg4(zj) = sum(ij,tmrg('25_LTRP',ij,zj,'01_KOR')) ;

* Supply of transport margin are given by the variable VST
 MRGNO(i,z)      = SUM{(marg,reg)$[j2comm(i,marg)$z2reg(z,reg)],
                       VST(marg,reg)};
...
*  Import duties are given by the variable TFRV
 ...
 TIMO(i,zj,z)$IMO(i,zj,z)
                 = SUM{(comm,regj,reg)$[j2comm(i,comm)
                       $z2reg(zj,regj)$z2reg(z,reg)],
                       TFRV(comm,regj,reg)};
...
* TIXO(i,z,zj)$IMO(i,z,zj)
 TIXO(i,z,zj)$IMO(i,z,zj)
                 = SUM{(comm,reg,regj)$[j2comm(i,comm)
...
* less subsidies (FBEP).
 ...
 TIWO(l,j,z)      = SUM{(endw,comm,reg)$[f2endw(l,endw)
                       $j2comm(j,comm)$z2reg(z,reg)],
                       FTRV(endw,comm,reg)+
                       FBEP(endw,comm,reg)};

 TIWO_J(l,z) = SUM(j, TIWO(l,j,z));

 TIKO(k,j,z)     = SUM{(endw,comm,reg)$[f2endw(k,endw)
                       $j2comm(j,comm)$z2reg(z,reg)],
                       FTRV(endw,comm,reg)+
                       FBEP(endw,comm,reg)};
...
* demand at agent prices and demand at market prices.
 ...
 TICO(i,z)       = SUM{(comm,reg)$[j2comm(i,comm)$z2reg(z,reg)],
...
* market prices (EVFB) and endowment at agent prices (EVOS)
 ...
 DTAX(f,z)          = SUM{(endw,reg)$[f2endw(f,endw)$z2reg(z,reg)],
                       SUM[comm,EVFB(endw,comm,reg)] -
                       SUM[comm,EVOS(endw, comm, reg)]} ;
...
* that is before taxes (EVFB).
 ...
 LDO(l,j,z)      = SUM{(endw,comm,reg)$[f2endw(l,endw)
                       $j2comm(j,comm)$z2reg(z,reg)],
                       EVFB(endw,comm,reg)};

 LDO_J(l,z)       = SUM(j,LDO(l,j,z)) - DTAX(l,z);

 RKDO(k,j,z)      = SUM{(endw,comm,reg)$[f2endw(k,endw)
                       $j2comm(j,comm)$z2reg(z,reg)],
                       EVFB(endw,comm,reg)};
...
*==============================================================================
 ...
 SH_VA(comm,j,z) Share of each sector TRAD_COM in aggregation j for region z
 VA_GTAP(comm,z) Value added using the GTAP sectors
;
...
* aggregated acording to the share of each commodity (QO) TRAD_COMM in total I
 ...
 Q_GTAP(comm,z)
                 = SUM{reg$z2reg(z,reg),
                   ...
                   VDGP(comm,reg)+VMGP(comm,reg)+
                   VDIP(comm,reg)+VMIP(comm,reg)+
                   SUM[commj,VDFP(comm,commj,reg)+
                       VMFP(comm,commj,reg)]};


 SH_Q(comm,i,z)$j2comm(i,comm)
                 = Q_GTAP(comm,z)/
                   SUM{commj$j2comm(i,commj),
                       Q_GTAP(commj,z)};

 sigma_M1(i,z)   = SUM{(comm,reg)$[j2comm(i,comm)$z2reg(z,reg)],
...
* aggregated acording to the share of total import (IMTO) TRAD_COMM in total I
 ...
 SH_IM(comm,i,z)$j2comm(i,comm)
                 = IM_GTAP(comm,z)/
                   SUM{commj$j2comm(i,commj),
                       IM_GTAP(commj,z)};

 sigma_M2(i,z)   = SUM{(comm,reg)$[j2comm(i,comm)$z2reg(z,reg)],
...
* acording to the share of each sector (VA) TRAD_COMM in total I
 ...
 VA_GTAP(comm,z)
                 = SUM{endw,
                       SUM[reg$z2reg(z,reg),EVFB(endw,comm,reg)]};

 SH_VA(comm,j,z)$j2comm(j,comm)
                 = VA_GTAP(comm,z)/
                   SUM{commj$j2comm(j,commj),
                       VA_GTAP(commj,z)};

 sigma_VA(j,z)   = SUM{(comm,reg)$[j2comm(j,comm)$z2reg(z,reg)],
                   ESUBVA(comm,reg)*SH_VA(comm,j,z)};

execute_unload 'DATA_AGG-2019_230403_POWER1',
...
*Benchmark variables and parameters used in PEP w-1 and PEP w-t
 CO, CGO, DDO, DEPO, DIO, DSO, EXO, EXTO, IMO, INVO, KSTO, LDO, MRGNO, POPO, RKDO,
 TDHO, DTAX, TICO, TIKO, TIMO, TIPO, TIWO, TIXO, tssm, tssd, tmrg, XSO, sigma_M1, sigma_M2,
 sigma_VA,Q_GTAP, SH_Q, ESUBD,  DIO2, TIMO2, IMO2, tmrg2, tmrg3, tmrg4, XSO_Matrix
;
