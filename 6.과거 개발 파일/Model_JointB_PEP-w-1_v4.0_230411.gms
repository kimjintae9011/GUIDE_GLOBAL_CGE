...
* rho_X0(j,Z)     Elasticity parameter (CET - exports and local sales)
 ...
 PowerShare(z)   Power Industiry's share of Aggregated electricity
...
*==============================================================================
 ...
 PO2(j,i,z)      Basic price of industry j's production of commodity i
 PO3(i,j,z)
 PPOWERO(z)
 PIO(i,z)
...
display PO, PO2, PO3, PIO, PTO, PPO, XSO ;
...
Display TnDShare, rho_X4, beta_X4, B_X4;
...
*==============================================================================
 ...
 P2(j,i,z)       Basic price of industry j's production of commodity i
 P3(i,j,z)       Basic price of power industry j's production of commodity i
 ...
 PT(j,z)         Basic price of industry j's output
...
* EQB_5(j,i,z)  Supply of exports for compsite activity
 ...
 EQ93(p_coal,j,z)        Industry coal product consumption in region z ktoe
 EQ94(p_oil,j,z)         Industry oil product consumption in region z ktoe
 EQ95(p_gas,j,z)         Industry gas product consumption in region z ktoe
 EQ96(p_oilproduct,j,z)  Industry oilproduct consumption in region z ktoe
 EQ97(p_elecheat,j,z)    Industry elecheat consumption in region z ktoe

 EQ98(p_coal,j,z)        Industry non energy coal product consumption in region z ktoe
 EQ99(p_oil,j,z)         Industry non energy oil product consumption in region z ktoe
 EQ100(p_gas,j,z)        Industry non energy gas product consumption in region z ktoe
 EQ101(p_oilproduct,j,z) Industry non energy oilproduct consumption in region z ktoe

 EQ102(p_coal,z)       Household coal product consumption in region z ktoe
 EQ103(p_gas,z)        Household gas product consumption in region z ktoe
 EQ104(p_oilproduct,z) Household oilproduct consumption in region z ktoe
 EQ105(p_elecheat,z)   Household elecheat consumption in region z ktoe

 EQ106(product,j,z)   Industry CO2 emission in region z ktCO2
 EQ107(product,z)     Household CO2 emission in region z ktCO2

 EQ108(product,j,z)   Industry CH4 emission in region z tCO2eq
 EQ109(product,z)     Household CH4 emission in region z tCO2eq

 EQ110(product,j,z)   Industry N2O emission in region z tCO2eq
 EQ111(product,z)     Household N2O emission in region z tCO2eq
...
*   5.3.5 Prices
*==============================================================================
 EQ49_1(j,z)..   PP(j,z)*XST(j,z) =e= PVA(j,z)*VA(j,z)+PCI(j,z)*CI(j,z);
...
*==============================================================================
*  6 (volume) energy variables
*==============================================================================
 EQ93(p_coal,j,z)$EEO(p_coal,j,z).. EE(p_coal,j,z) =e= EEI(p_coal,j,z)*DI('02_COAL',j,z) ;

 EQ94(p_oil,j,z)$EEO(p_oil,j,z).. EE(p_oil,j,z) =e= EEI(p_oil,j,z)*DI('03_OIL',j,z) ;

 EQ95(p_gas,j,z)$EEO(p_gas,j,z).. EE(p_gas,j,z) =e= EEI(p_gas,j,z)*DI('04_GAS',j,z) ;

 EQ96(p_oilproduct,j,z)$EEO(p_oilproduct,j,z).. EE(p_oilproduct,j,z) =e= EEI(p_oilproduct,j,z)*DI('10_PETROLCOAL',j,z) ;

 EQ97(p_elecheat,j,z)$EEO(p_elecheat,j,z).. EE(p_elecheat,j,z) =e= EEI(p_elecheat,j,z)*DI('18_ELEC',j,z) ;

 EQ98(p_coal,j,z)$NEO(p_coal,j,z).. NE(p_coal,j,z) =e= NEI(p_coal,j,z)*DI('02_COAL',j,z) ;

 EQ99(p_oil,j,z)$NEO(p_oil,j,z).. NE(p_oil,j,z) =e= NEI(p_oil,j,z)*DI('03_OIL',j,z) ;

 EQ100(p_gas,j,z)$NEO(p_gas,j,z).. NE(p_gas,j,z) =e= NEI(p_gas,j,z)*DI('04_GAS',j,z) ;

 EQ101(p_oilproduct,j,z)$NEO(p_oilproduct,j,z).. NE(p_oilproduct,j,z) =e= NEI(p_oilproduct,j,z)*DI('10_PETROLCOAL',j,z) ;

 EQ102(p_coal,z)$EHO(p_coal,z).. EH(p_coal,z) =e= EHI(p_coal,z)*C('02_COAL',z) ;

 EQ103(p_gas,z)$EHO(p_gas,z)..  EH(p_gas,z) =e= EHI(p_gas,z)*C('04_GAS',z) ;

 EQ104(p_oilproduct,z)$EHO(p_oilproduct,z).. EH(p_oilproduct,z) =e= EHI(p_oilproduct,z)*C('10_PETROLCOAL',z) ;

 EQ105(p_elecheat,z)$EHO(p_elecheat,z).. EH(p_elecheat,z) =e= EHI(p_elecheat,z)*C('18_ELEC',z) ;

*==============================================================================
*  7 (volume) CO2 emission variables
*==============================================================================
 EQ106(product,j,z)$CO2IO(product,j,z)..  CO2I(product,j,z) =E= EE(product,j,z)*41.868*GHGsEF(product,'CO2EF')*1*(44/12)*0.001*GWP('CO2EF') ;

 EQ107(product,z)$CO2HO(product,z)..  CO2H(product,z) =E= EH(product,z)*41.868*GHGsEF(product,'CO2EF')*1*(44/12)*0.001*GWP('CO2EF') ;

 EQ108(product,j,z)$CH4IO(product,j,z)..  CH4I(product,j,z) =E= EE(product,j,z)*41.868*GHGsEF(product,'CH4EF')*1000*0.000001*GWP('CH4EF') ;

 EQ109(product,z)$CH4HO(product,z)..  CH4H(product,z) =E= EH(product,z)*41.868*GHGsEF(product,'CH4EF')*1000*0.000001*GWP('CH4EF') ;

 EQ110(product,j,z)$N2OIO(product,j,z)..  N2OI(product,j,z) =E= EE(product,j,z)*41.868*GHGsEF(product,'N2OEF')*1000*0.000001*GWP('N2OEF') ;

 EQ111(product,z)$N2OHO(product,z)..  N2OH(product,z) =E= EH(product,z)*41.868*GHGsEF(product,'N2OEF')*1000*0.000001*GWP('N2OEF') ;
