...
*==============================================================================
Parameters
 CO2FACTOR(ene,j,z)     CO2 emissions factor (tCO2 per $)
 CTAX0(z)               initial Carbon tax $ per ton CO2
 TCTAX0(z)              initial Government Revenue from Carbon tax
;

 CO2FACTOR(ene,j,z) = 0 ;
 CO2FACTOR('02_COAL',j,z)$DEO('02_COAL',j,z) = [sum(p_coal,CO2IO(p_coal,j,z))/DEO('02_COAL',j,z)]*(1000/(10*10**9));
 CO2FACTOR('04_GAS',j,z)$DEO('04_GAS',j,z) = [sum(p_gas,CO2IO(p_gas,j,z))/DEO('04_GAS',j,z)]*(1000/(10*10**9));
 CO2FACTOR('10_PETROLCOAL',j,z)$DEO('10_PETROLCOAL',j,z) = [sum(p_oilproduct,CO2IO(p_oilproduct,j,z))/DEO('10_PETROLCOAL',j,z)]*(1000/(10*10**9));
...
* EQ9_1(ene,j,z)..  DE(ene,j,z) =e= [beta_ENER(ene,j,z)*PCE(j,z)/((1+CTAX(Z)*CO2FACTOR(ene,j,z))*P4(ene,j,z))]
*                                   **sigma_ENER(j,z)*B_ENER(j,z)**(sigma_ENER(j,z)-1)
*                                   *CE(j,z);
...
*==============================================================================
 ...
 EQ13(z)..       YDH(z) =e= YH(z)-TDH(z)+TCTAX(Z);
...
*============================================================================== 
$ontext
 LD.LO(l,j,z)   = 0.00000001*LD.l(l,j,z);
 LDC.LO(j,z)    = 0.00000001*LDC.l(j,z);
 WTI.LO(l,j,z)  = 0.00000001*WTI.l(l,j,z); 
 WC.LO(j,z)     = 0.00000001*WC.l(j,z);
* RC.LO(j,z)     = 0.00000001*RC.l(j,z);
* R.LO(k,j,z)    = 0.00001*R.l(k,j,z);
* RTI.LO(k,j,z)  = 0.00001*RTI.l(k,j,z);
 ...
 PKLE.LO(j,z)   = 0.00001*PKLE.L(j,z);
...
*KD.fx(k,power,'01_KOR') = KDO(k,power,'01_KOR')*0.7 ;
...
;
$INCLUDE RESULTS_PEP-w-1_v4.0_231208.gms
