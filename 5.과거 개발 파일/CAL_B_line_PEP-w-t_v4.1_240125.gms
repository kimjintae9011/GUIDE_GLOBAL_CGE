...
* Investment by destination follows:
 ...
 INVO3(Z) = sum(i,INVO(i,z)*PCO(i,z));

* INDO(k,j,z)$KDO(k,j,z) = ror(z)*KKO(k,j,z);

display phiO, UO, KKO, RO, KSTO, INDO, ITO, INVO, INVO3   ;
*$Exit
*==============================================================================
...
* EQ99(z,t)..                 IT(z,t) =e= sum((k,j)$KDO(k,j,z),IND(k,j,z,t)); 
...
* ttdh0.fx(z,time)    = ttdh0O(z)*exogro(z,time);
 ...
 ttim.fx(i,zj,z,time) = ttimO(i,zj,z)*0.8;
...
PARAMETER
 ...
 KKT(z,time)     Total Capital Sthock
;
...
* KKT(z,time)             = sum((k,j), KK.L(k,j,z,time));

execute_unload 'B_line',
 A_VA_RES, INDX, delta, XST, VA, LS, KS, LD, KD, IND, sigma_LD, EX, g_GDP, g_POP, GDP_BP, RC, IT, SH, SG, CABX, KK, R, PK;
