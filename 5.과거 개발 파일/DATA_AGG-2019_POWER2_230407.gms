...
*==============================================================================
$CALL har2gdx basedata2019_230407_POWER2 basedata2019.gdx
$CALL har2gdx parameter2019_POWER2.prm Parameters2019.gdx
...
/
 ...
 23_eWind       Solar Hydro Other generation
 24_eSHo        Solar Hydro Other generation
 25_CONSTRUC    Construction
 26_LTRP        Land transport service(road rail)
 27_WTRP        Water transport service
 28_ATRP        Air transport service
 29_SER         Service
/
...
/
 ...
 27_WTRP.         (wtp)
 28_ATRP.         (atp)
 29_SER.          (trd, afs, whs, cmn, ofi, ins, rsa, obs, ros, dwe, osg, edu, hht, wtr)
/
...
*lt Strictly less than
 IMO('23_eWind',zj, z)$(IMO('23_eWind', zj, z) lt 1e-05) = 0.000001 ;
...
*                       SUM[marg,VTMFSD(marg,comm,regj,reg)]};
  ...
  tmrg('26_LTRP',ij,zj,z)$IMO(ij,zj,z)
                 = SUM{(comm,regj,reg)$[j2comm(ij,comm)
                       $z2reg(zj,regj)$z2reg(z,reg)],
                       VTMFSD('otp',comm,regj,reg)};

  tmrg('27_WTRP',ij,zj,z)$IMO(ij,zj,z)
                 = SUM{(comm,regj,reg)$[j2comm(ij,comm)
                       $z2reg(zj,regj)$z2reg(z,reg)],
                       VTMFSD('wtp',comm,regj,reg)};

 tmrg('28_ATRP',ij,zj,z)$IMO(ij,zj,z)
                 = SUM{(comm,regj,reg)$[j2comm(ij,comm)
                       $z2reg(zj,regj)$z2reg(z,reg)],
                       VTMFSD('atp',comm,regj,reg)};

 tmrg3(ij,zj) = tmrg('26_LTRP',ij,zj,'01_KOR') ;

 tmrg4(zj) = sum(ij,tmrg('26_LTRP',ij,zj,'01_KOR')) ;
...
execute_unload 'DATA_AGG-2019_230403_POWER2',
