...
* rho_X0(j,Z)     Elasticity parameter (CET - exports and local sales)
 ...
 v2(j,z)         Share parameter (Leontief)
...
*PCEO(j,z)       = SUM[ene,PO4(ene,j,z)*DEO(ene,j,z)]/CEO(j,z) ; 
...
*==============================================================================
 ...
 LDO2(l,j,z)   = [beta_LD(l,j,z)*WCO(j,z)/WTIO(l,j,z)]
...
*                 = [PO4(ene,j,z)*ENERO(ene,j,z)**(1+rho_ENER(j,z))]/
 ...
 DEO2(ene,j,z)   = [beta_ENER(ene,j,z)*PCEO(j,z)/PO4(ene,j,z)]
                               **sigma_ENER(j,z)*B_ENER(j,z)**(sigma_ENER(j,z)-1)
                               *CEO(j,z);

execute_unload 'Composite_E',
rho_ENER, beta_ENER, B_ENER, DEO, DIO, DEO2,  LDO, LDO2, sigma_LD, sigma_ENER, WCO, WTIO, PCEO, PO4 ;
...
execute_unload 'KLE',
rho_KLE, beta_KLE, B_KLE, VAO, CEO, KLEO, PKLEO, LDCO, KDCO ;
...
* EQ1(j,z)         Leontief - demand for value added
 EQ1_1(j,z)         Leontief - demand for value added
...
* EQ49_3(j,i,z)   Basic price of industry j's production of commodity i
 ...
 EQ51_1(j,z)      Price index of industry j intermediate consumption
...
* EQ1(j,z)..      VA(j,z) =e= v(j,z)*XST(j,z);
 ...
 EQ1_1(j,z)..    KLE(j,z) =e= v2(j,z)*XST(j,z);
...
* EQ3_1(j,z)..     KLE(j,z) =e= B_KLE(j,z)*{
*                    [beta_KLE(j,z)*VA(j,z)**(-rho_KLE(j,z))]$VAO(j,z)
*                   +[(1-beta_KLE(j,z))*CE(j,z)**(-rho_KLE(j,z))]$CEO(j,z)
*                                                   }**(-1/rho_KLE(j,z));
 ...
 EQ4_1(j,z)$[VAO(j,z) and CEO(j,z)]..
                 VA(j,z) =e= {[beta_KLE(j,z)/(1-beta_KLE(j,z))]
                              *[PCE(j,z)/PVA(j,z)]}**sigma_KLE(j,z)*CE(j,z);
...
* EQ9_1(ene,j,z)..  DE(ene,j,z) =e= aij2(ene,j,z)*CE(j,z) ;
 ...
 EQ9_2(j,z)..      CE(j,z) =e= B_ENER(j,z)*SUM[ene$DEO(ene,j,z),beta_ENER(ene,j,z)
                                *DE(ene,j,z)**(-rho_ENER(j,z))]**(-1/rho_ENER(j,z));
...
* EQ49(j,z)..     PP(j,z)*XST(j,z) =e= PVA(j,z)*VA(j,z)+PCI(j,z)*CI(j,z)+PCE(j,z)*CE(j,z);
 ...
 EQ49(j,z)..     PP(j,z)*XST(j,z) =e= PKLE(j,z)*KLE(j,z)+PCI(j,z)*CI(j,z);
...
* EQ51_1(j,z)..   PCE(j,z)*CE(j,z) =e= SUM[ene,P4(ene,j,z)*DE(ene,j,z)];
 ...
 EQ52_1(j,z)..   PKLE(j,z)*KLE(j,z) =e= [PVA(j,z)*VA(j,z)]$VAO(j,z)
                                       +[PCE(j,z)*CE(j,z)]$CEO(j,z);
