...
$GDXIN DATA_AGG-2019_240125_w-t.gdx
...
*==============================================================================
 ...
 INV_ASS(z)           Required investment for the assumed growth
 INV_SAM(z)           Observed investment in the SAM'
 INVO2(i,z)           
 CGO2(i,z)
 adj(z)               II_ASS vs. III_SAM [>1:more than actual]
 growthz(z)           Steady state grwoth
 dep(z)               Depriciation rate
 ror(z)               Rate of return of capital
 zeta
...
* =============================================================================
 INV_ASS(z) = (growthz(z) + dep(z))/ror(z)*SUM[(k,j),RKDO(k,j,z)];
 INV_SAM(z) = sum(i, INVO(i,z));
 adj(z)     = INV_ASS(z)/INV_SAM(z);

 INVO2(i,z) = INVO(i,z);
 CGO2(i,z)  = CGO(i,z);

* Adjusting investment level
 INVO(i,z)  = INVO(i,z)*adj(z);

* Reallocating the gap made by the inv. adjustment to gov. cons.
 CGO(i,z)   = CGO2(i,z) - (INVO(i,z) - INVO2(i,z));

display INV_ASS, INV_SAM, adj, INVO, INVO2, CGO2, CGO ;
...
* Investment by destination follows:
 ...
 KKO(k,j,z)$KDO(k,j,z) = KDO(k,j,z)/ror(z);

 phiO(z)         = [ITO(z)/PKO(z)]/KSTO(z);

 INDO(k,j,z)$KDO(k,j,z) = KDO(k,j,z)/sum((kj,jj),KDO(kj,jj,z))*ITO(z);
 
* INDO(k,j,z)$KDO(k,j,z) = ror(z)*KKO(k,j,z);

display phiO, UO, KKO, RO, KSTO, INDO ;
*$Exit
*==============================================================================
...
*$INCLUDE DATA_WEB-2019_240117.gms
...
execute_unload 'CO2FACTOR_wKLEM',
...
* EQ97b(z,time)           User cost of capital
 ...
 EQ98(k,j,z,time)         Capital market
* EQ99(z,time)             Investment good market clearing condition
 EQ100(k,j,z,time)        Sectoral investment allocation
...
*==============================================================================
 ...
 EQ9_2(ene,j2,z,t)..  DE(ene,j2,z,t) =e= [beta_ENER(ene,j2,z)*PCE(j2,z,t)/((P4(ene,j2,z,t)+CTAX(z,t)*CO2FACTOR(ene,j2,z)))]
...
* EQ97b(z,t)..    U(z,t) =e= PK(z,t)*(delta(z)+IR(z,t));
 ...
 EQ98(k,j,Z,t)$KDO(k,j,z)..  KD(k,j,z,t) =e= ror(z)*KK(k,j,z,t);

* EQ99(z,t)..      IT(z,t) =e= PK(z,t)*sum((k,j)$KDO(k,j,z),IND(k,j,z,t)); 

 EQ100(k,j,z,t)$KDO(k,j,z).. PK(z,t)*IND(k,j,z,t) =e= [R(k,j,z,t)**zeta*KD(k,j,z,t)/sum((kj,jj)$KDO(kj,jj,z),R(k,j,z,t)**zeta*KD(kj,jj,z,t))]*IT(z,t);    
...
*==============================================================================
$INCLUDE BAU_SOLVE_240125.gms
$INCLUDE BAU_RESULTS_240125.gms
...
*==============================================================================
*$INCLUDE SIM_SOLVE_240125.gms
*$INCLUDE SIM_RESULTS_240125.gms
