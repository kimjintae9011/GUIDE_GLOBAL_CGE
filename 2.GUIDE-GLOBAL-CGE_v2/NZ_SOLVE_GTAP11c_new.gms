...
* 2040년 이전: 기존 집약도 유지
CO2FACTOR2_Star(ene,j5,PERMIT_Z,time)$[ord(time) gt 2 and ord(time) lt 22] = CO2FACTOR(ene,j5,PERMIT_Z);

* 2040년(22) ~ 2050년(32): 선형적으로 감소
CO2FACTOR2_Star(ene,j5,PERMIT_Z,time)$[ord(time) ge 22 and ord(time) le 32] = 
    CO2FACTOR(ene,j5,PERMIT_Z) * (1 - ( (1 - residual_ratio) * (ord(time) - 22) / (32 - 22) ));

* 2050년 이후: 잔존 비율 유지
CO2FACTOR2_Star(ene,j5,PERMIT_Z,time)$[ord(time) gt 32] = CO2FACTOR(ene,j5,PERMIT_Z) * residual_ratio;
