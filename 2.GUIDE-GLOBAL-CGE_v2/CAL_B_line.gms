...
*==============================================================================
* Begin LOOP over time periods for Baseline Calibration
*==============================================================================
LOOP[time,
    T(time) = YES;
...
* Exogenous tax rates and basic variables
...
* and multifactor productivity (A_VA) is endogenous.
    GDP_BP_REAL.fx(z,t1) = GDP_BP_REALO(z);
    GDP_BP_REAL.fx(z,time)$[ord(time) gt 1] = GDP_BP_REAL.l(z,time-1) * [1 + g_GDP(z,time)];

    A_VA.L(z,t1) = 1;
    A_VA.L(z,time)$[ord(time) gt 1] = A_VA.L(z,time-1);

* Domestic savings rates are made to follow the evolution anticipated by FBQF
* and the intercept for the household savings function is endogenously determined.
    sh1.fx(z,t1) = sh1O(z);
    sh1.fx(z,time)$[ord(time) gt 1] = sh1.l(z,time-1) * [1 + g_SDR(z,time-1)];

    sh0.l(z,t1) = sh0O(z);
    sh0.l(z,time)$[ord(time) gt 1] = sh0.l(z,time-1) * [1 + growthz(z)];

*==============================================================================
* 6.1.4 Other exogenous variables fixed to their lagged values
*==============================================================================
    CABX.FX(z1,time) = CABXO(z1) * cabix(z1,time);
                    
    CMIN.FX(i,z,t1) = CMINO(i,z);
    CMIN.FX(i,z,time)$[ord(time) gt 1] = CMIN.l(i,z,time-1) * [1 + g_POP(z,time)];

    KD.fx(k,j,z,t1)$KDO(k,j,z) = KDO(k,j,z);
    KD.fx(k,j,z,time)${[ord(time) gt 1] and KDO(k,j,z)} = KD.l(k,j,z,time-1) * [1 - delta(z)] + IND.l(k,j,z,time-1);
...
*==============================================================================
* Output Variables for the Baseline
*==============================================================================
PARAMETER
...
execute_unload 'Input_CGE/B_line.gdx', A_VA_RES, GX, G_REALX, sh1X, sh0X ;