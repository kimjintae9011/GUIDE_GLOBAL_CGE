...
* --- [1] Scale Parameters (CES / CET) ---
 ...
 B_X5(Z_GRN)                  Scale parameter (CES - Aggregated Ironstl)
 B_VAT(Z_GRN)                 Scale parameter for VAT nest
 B_VAT_t(Z_GRN,time)          Scale parameter for VAT nest over time
...
* --- [2] Share Parameters ---
 ...
 beta_X5(ironstl,z)           Share parameter (CES - Aggregated Ironstl)
 beta_X5_t(ironstl,z,time)    Share parameter (CES - Aggregated Ironstl)
 ...
 beta_VAT(Z_GRN)              Share parameter for VA in VAT nest
...
* --- [3] Elasticity Parameters (rho, sigma, elas) ---
 ...
 rho_X5(Z_GRN)                Elasticity parameter (CES - Aggregated Ironstl)
 rho_VAT(Z_GRN)               Substitution parameter for VAT nest
 ...
 sigma_X5(Z_GRN)              Elasticity (CES - Aggregated IRONSTL)
 sigma_Y(i,z)                 Income elasticity of consumption
 sigma_VAT(Z_GRN)             Elasticity of substitution between VA and TSF
...
* --- [7] Other Coefficients & Rescaling ---
 ...
 v_tsf(j,Z_GRN)               Leontief input coefficient for TSF
 TSFS(Z_GRN,time)             Supply of TSF
 VATO(Z_GRN)                  Base year quantity of VAT composite
 PVATO(Z_GRN)                 Base year price of VAT composite    
 v_vat(Z_GRN)                 Leontief coefficient for VAT composite to XST
;
...
PARAMETER
 ...
 IRONSTLQO(Z_GRN)    Total iron and steel output
 Share_Elec(j2,z)
...
PARAMETER
 ...
 PIRONSTLO(Z_GRN)    Basic price of composite ironstl commodity in region z
...
* CES - composite capital
  ...
  sigma_KD('32_BSIRONSTL',z)   = 2 * sigma_VA('13_IRONSTL',z);

* CES - composite labor
 sigma_LD(j,z)                 = 2 * sigma_VA(j,z); 
 sigma_LD('32_BSIRONSTL',z)    = 2 * sigma_VA('13_IRONSTL',z);
 sigma_VA('13_IRONSTL',z)      = 0.5;
...
*==============================================================================
 ...
 YHO(z)           = YHLO(z) + YHKO(z) + SUM(Z_GRN$sameas(z, Z_GRN), PTSFO(Z_GRN) * TSFO(Z_GRN));
 YDHO(z)          = YHO(z) - TDHO(z);
 CTHO(z)          = SUM[i, CO(i,z)];
 SHO(z)           = YDHO(z) - CTHO(z);
...
*==============================================================================
 ...
 PO2('32_BSIRONSTL','13_IRONSTL',Z_GRN) = Green_Markup(Z_GRN);
 ...
 PPO(j,z)$XSTO(j,z) 
    = ( TIWO(j,z) + WO(z) * LDO(j,z)
      ...
      + SUM(Z_GRN$sameas(z, Z_GRN), PTSFO(Z_GRN) * TSFO(Z_GRN))$sameas(j,'32_BSIRONSTL')
...
*==============================================================================
 PKO(z)           = 1;
 KSTO(z)          = KSTO(z) / PKO(z);
 KSTO(z)          = KSTO(z) - SUM(Z_GRN$sameas(z, Z_GRN), PTSFO(Z_GRN) * TSFO(Z_GRN));
...
*==============================================================================
 ...
 v_vat(Z_GRN)     = VATO(Z_GRN) / XSTO('32_BSIRONSTL', Z_GRN);
 B_VAT_t(Z_GRN,time) = B_VAT(Z_GRN);
...
*==============================================================================
 ...
 GDP_IBO(z)     
    ...
    + SUM(Z_GRN$sameas(z, Z_GRN), PTSFO(Z_GRN) * TSFO(Z_GRN))
    + TPRODNO(z) 
    + TPRCTSO(z);
...
*==============================================================================
 ...
 CALEQ1(z,t)..    
    SH(z,t)+SG(z,t) =e= sh1(z,t)*GDP_IB(z,t);
...
*==============================================================================
 ...
 CALEQ2(z,t)..     
    G(z,t) =e= GO(z)*GDP_BP(z,t)/GDP_BPO(z);
...
*==============================================================================
 ...
 CALEQ3(k,pub,z,t)$KDO(k,pub,z)..
    IND(k,pub,z,t)*PK(z,t) =e= INDO(k,pub,z)*PKO(z)*GDP_BP(z,t)/GDP_BPO(z);
