...
* --- 5. Energy Demand & Supply ---
 ...
 valEE(p_coal,j,z,time,'%CurrentSce%')       =  valAEEI(z,time,'%CurrentSce%')*EEI(p_coal,j,z)*DE.L('02_COAL',j,z,time);
 valEE(p_oil,j,z,time,'%CurrentSce%')        = valAEEI(z,time,'%CurrentSce%')*EEI(p_oil,j,z)*DE.L('03_OIL',j,z,time);
 valEE(p_gas,j,z,time,'%CurrentSce%')        = valAEEI(z,time,'%CurrentSce%')*EEI(p_gas,j,z)*DE.L('04_GAS',j,z,time);
 valEE(p_oilproduct,j,z,time,'%CurrentSce%') = valAEEI(z,time,'%CurrentSce%')*EEI(p_oilproduct,j,z)*DE.L('10_PETROLCOAL',j,z,time);
 valEE(p_elecheat,j,z,time,'%CurrentSce%')   = valAEEI(z,time,'%CurrentSce%')*EEI(p_elecheat,j,z)*DE.L('18_ELEC',j,z,time);
 ...
 valNE(p_gas,j,z,time,'%CurrentSce%')        = valAEEI(z,time,'%CurrentSce%')*NEI(p_gas,j,z)*DE.L('04_GAS',j,z,time);
 valNE(p_oilproduct,j,z,time,'%CurrentSce%') = valAEEI(z,time,'%CurrentSce%')*NEI(p_oilproduct,j,z)*DE.L('10_PETROLCOAL',j,z,time);
 valNE(p_elecheat,j,z,time,'%CurrentSce%')   = valAEEI(z,time,'%CurrentSce%')*NEI(p_elecheat,j,z)*DE.L('18_ELEC',j,z,time);
 
 valEH(p_coal,z,time,'%CurrentSce%')         = valAEEI(z,time,'%CurrentSce%')*EHI(p_coal,z)*C.L('02_COAL',z,time);
 valEH(p_oil,z,time,'%CurrentSce%')          = valAEEI(z,time,'%CurrentSce%')*EHI(p_oil,z)*C.L('03_OIL',z,time);
 valEH(p_gas,z,time,'%CurrentSce%')          = valAEEI(z,time,'%CurrentSce%')*EHI(p_gas,z)*C.L('04_GAS',z,time); 
 valEH(p_oilproduct,z,time,'%CurrentSce%')   = valAEEI(z,time,'%CurrentSce%')*EHI(p_oilproduct,z)*C.L('10_PETROLCOAL',z,time);
 valEH(p_elecheat,z,time,'%CurrentSce%')     = valAEEI(z,time,'%CurrentSce%')*EHI(p_elecheat,z)*C.L('18_ELEC',z,time);
...
valTES_coal(z,time,'%CurrentSce%')$(valQ('02_COAL',z,'2019','%CurrentSce%') > 0)
    = sum(p_coal, TES_Coal(p_coal, z)) 
      * (valQ('02_COAL',z,time,'%CurrentSce%') / valQ('02_COAL',z,'2019','%CurrentSce%')); 

valTES_gas(z,time,'%CurrentSce%')$(valQ('04_GAS',z,'2019','%CurrentSce%') > 0)
    = sum(p_gas, TES_Gas(p_gas, z)) 
      * (valQ('04_GAS',z,time,'%CurrentSce%') / valQ('04_GAS',z,'2019','%CurrentSce%')); 

valTES_oil(z,time,'%CurrentSce%')
    = [ sum(p_oil, TES_Oil(p_oil, z)) 
        * (valQ('03_OIL',z,time,'%CurrentSce%') / max(eps, valQ('03_OIL',z,'2019','%CurrentSce%'))) ]
    + [ sum(p_oilproduct, TES_Oilproduct(p_oilproduct, z)) 
        * (valQ('10_PETROLCOAL',z,time,'%CurrentSce%') / max(eps, valQ('10_PETROLCOAL',z,'2019','%CurrentSce%'))) ]; 

valTES_nuc(z,time,'%CurrentSce%')$(valXST('19_eNuclear',z,'2019','%CurrentSce%') > 0)
    = sum(p_nuclear, TES_Nuclear(p_nuclear, z))
      * (valXST('19_eNuclear',z,time,'%CurrentSce%') / valXST('19_eNuclear',z,'2019','%CurrentSce%'));

valTES_wind(z,time,'%CurrentSce%')$(valXST('23_eWind',z,'2019','%CurrentSce%') > 0)
    = sum(p_wind, TES_Wind(p_wind, z))
      * (valXST('23_eWind',z,time,'%CurrentSce%') / valXST('23_eWind',z,'2019','%CurrentSce%'));

valTES_solar(z,time,'%CurrentSce%')$(valXST('24_eSolar',z,'2019','%CurrentSce%') > 0)
    = sum(p_solar, TES_Solar(p_solar, z))
      * (valXST('24_eSolar',z,time,'%CurrentSce%') / valXST('24_eSolar',z,'2019','%CurrentSce%'));

valTES_hydro(z,time,'%CurrentSce%')$(valXST('25_eHydro',z,'2019','%CurrentSce%') > 0)
    = sum(p_hydro, TES_Hydro(p_hydro, z))
      * (valXST('25_eHydro',z,time,'%CurrentSce%') / valXST('25_eHydro',z,'2019','%CurrentSce%'));

valTES_bio(z,time,'%CurrentSce%')$(valXST('26_eOther',z,'2019','%CurrentSce%') > 0)
    = sum(p_bio, TES_Bio(p_bio, z))
      * (valXST('26_eOther',z,time,'%CurrentSce%') / valXST('26_eOther',z,'2019','%CurrentSce%'));

valTES_waste(z,time,'%CurrentSce%')$(valXST('26_eOther',z,'2019','%CurrentSce%') > 0)
    = sum(p_waste, TES_Waste(p_waste, z))
      * (valXST('26_eOther',z,time,'%CurrentSce%') / valXST('26_eOther',z,'2019','%CurrentSce%'));

valTES_geo(z,time,'%CurrentSce%')$(valXST('26_eOther',z,'2019','%CurrentSce%') > 0)
    = sum(p_geo, TES_Geo(p_geo, z))
      * (valXST('26_eOther',z,time,'%CurrentSce%') / valXST('26_eOther',z,'2019','%CurrentSce%'));

valTES_other(z,time,'%CurrentSce%')$(valXST('26_eOther',z,'2019','%CurrentSce%') > 0)
    = sum(p_other_power, TES_Other(p_other_power, z))
      * (valXST('26_eOther',z,time,'%CurrentSce%') / valXST('26_eOther',z,'2019','%CurrentSce%'));

valTES(z,time,'%CurrentSce%') = 
    valTES_coal(z,time,'%CurrentSce%') + valTES_oil(z,time,'%CurrentSce%') + valTES_gas(z,time,'%CurrentSce%')
  + valTES_nuc(z,time,'%CurrentSce%')  + valTES_hydro(z,time,'%CurrentSce%') + valTES_solar(z,time,'%CurrentSce%')
  + valTES_wind(z,time,'%CurrentSce%') + valTES_bio(z,time,'%CurrentSce%')   + valTES_waste(z,time,'%CurrentSce%')
  + valTES_geo(z,time,'%CurrentSce%')  + valTES_other(z,time,'%CurrentSce%');
...
* --- 6. Emissions & Climate Policy (CO2, CH4, N2O, F-gases) ---
 ...
 valEndo_TotalCO2(z,time,'%CurrentSce%')        = (sum((ene,j),valEndo_CO2IND(ene,j,z,time,'%CurrentSce%')) + sum(ene, valEndo_CO2HH(ene,z,time,'%CurrentSce%'))-valQDAC(z,time,'%CurrentSce%')*1000);
 valEndo_GlobalTotalCO2(time,'%CurrentSce%')  = sum(z,valEndo_TotalCO2(z,time,'%CurrentSce%')) ;
 valCO2I(p_coal,j,z,time,'%CurrentSce%') = max(0, valEE(p_coal,j,z,time,'%CurrentSce%') - (valNE(p_coal,j,z,time,'%CurrentSce%') * GHGsEF(p_coal,'Stored_rate'))) * 41.868 * GHGsEF(p_coal,'CO2EF') * (44/12)* 0.001* valEndoAbate_IND('02_COAL',j,z,time,'%CurrentSce%') ;
 valCO2I(p_oilproduct,j,z,time,'%CurrentSce%') = max(0, valEE(p_oilproduct,j,z,time,'%CurrentSce%') - (valNE(p_oilproduct,j,z,time,'%CurrentSce%') * GHGsEF(p_oilproduct,'Stored_rate')))* 41.868 * GHGsEF(p_oilproduct,'CO2EF') * (44/12) * 0.001* valEndoAbate_IND('10_PETROLCOAL',j,z,time,'%CurrentSce%') ;
 valCO2I(p_gas,j,z,time,'%CurrentSce%') = max(0, valEE(p_gas,j,z,time,'%CurrentSce%') - (valNE(p_gas,j,z,time,'%CurrentSce%') * GHGsEF(p_gas,'Stored_rate')))* 41.868 * GHGsEF(p_gas,'CO2EF') * (44/12) * 0.001* valEndoAbate_IND('04_GAS',j,z,time,'%CurrentSce%') ;
 valCO2I(p_oil,j,z,time,'%CurrentSce%') $(not sameas(j, '10_PETROLCOAL')) = valEE(p_oil,j,z,time,'%CurrentSce%') * 41.868 * GHGsEF(p_oil,'CO2EF') * (44/12) * 0.001* valEndoAbate_IND('03_OIL',j,z,time,'%CurrentSce%') ;

 valCO2I(p_irongas,'13_IRONSTL',z,time,'%CurrentSce%') = -1 * Gas_Prod_Iron(p_irongas, z)*( valXST('13_IRONSTL',z,time,'%CurrentSce%') /valXST('13_IRONSTL',z,'2019','%CurrentSce%')) * 41.868 * GHGsEF(p_irongas,'CO2EF') * (44/12) * 0.001 ;
 valCO2I(p_oilproduct,'13_IRONSTL',z,time,'%CurrentSce%') =-1 * Coke_Prod_Iron(p_oilproduct, z)*( valXST('13_IRONSTL',z,time,'%CurrentSce%') /valXST('13_IRONSTL',z,'2019','%CurrentSce%')) * 41.868 * GHGsEF(p_oilproduct,'CO2EF') * (44/12)*0.001;

 valCO2NE(p_coal,j,z,time,'%CurrentSce%') =  (valNE(p_coal,j,z,time,'%CurrentSce%') * GHGsEF(p_coal,'Stored_rate')) * 41.868 * GHGsEF(p_coal,'CO2EF') * (44/12)* 0.001* GHGsEF(p_coal,'CO2EF') * (44/12)* 0.001* valEndoAbate_IND('02_COAL',j,z,time,'%CurrentSce%') ;
 valCO2NE(p_oilproduct,j,z,time,'%CurrentSce%') =  (valNE(p_oilproduct,j,z,time,'%CurrentSce%') * GHGsEF(p_oilproduct,'Stored_rate'))* 41.868 * GHGsEF(p_oilproduct,'CO2EF') * (44/12) * 0.001* valEndoAbate_IND('10_PETROLCOAL',j,z,time,'%CurrentSce%') ;
 valCO2NE(p_gas,j,z,time,'%CurrentSce%') =  (valNE(p_gas,j,z,time,'%CurrentSce%') * GHGsEF(p_gas,'Stored_rate'))* 41.868 * GHGsEF(p_gas,'CO2EF') * (44/12) * 0.001* valEndoAbate_IND('04_GAS',j,z,time,'%CurrentSce%') ;

 valCO2H(p_coal,z,time,'%CurrentSce%')          = valEH(p_coal,z,time,'%CurrentSce%')*41.868*GHGsEF(p_coal,'CO2EF')*(44/12)*0.001* valEndoAbate_HH('02_COAL',z,time,'%CurrentSce%') ;
 valCO2H(p_oil,z,time,'%CurrentSce%')            = valEH(p_oil,z,time,'%CurrentSce%')*41.868*GHGsEF(p_oil,'CO2EF')*(44/12)*0.001* valEndoAbate_HH('03_OIL',z,time,'%CurrentSce%') ;
 valCO2H(p_gas,z,time,'%CurrentSce%')          = valEH(p_gas,z,time,'%CurrentSce%')*41.868*GHGsEF(p_gas,'CO2EF')*(44/12)*0.001* valEndoAbate_HH('04_GAS',z,time,'%CurrentSce%') ;
 valCO2H(p_oilproduct,z,time,'%CurrentSce%') = valEH(p_oilproduct,z,time,'%CurrentSce%')*41.868*GHGsEF(p_oilproduct,'CO2EF')*(44/12)*0.001 * valEndoAbate_HH('10_PETROLCOAL',z,time,'%CurrentSce%') ;
...
* ====== Global GHGs ======
 ...
 valGlobal_GHG(time,'%CurrentSce%')              = valGlobal_CO2(time,'%CurrentSce%') +valGlobal_CH4(time,'%CurrentSce%')+valGlobal_N2O(time,'%CurrentSce%') ;
...
* --- 9. Population ---
 valPOP(z,time,'%CurrentSce%') =   TOT_POP(z,time) ;
...
* To reduce the .gdx file size
 ...
 valGDP_FD, valGDP_IB, valGDP_MP, valGDP_MP_REAL, valIMT,  valINV, 
 valIR, valIT, valIT_REAL,  valKDC, valKS, valLD, valLDC, valLS, valLST, 
 valMRGN, valP, valPC, valPCI, valPCE, valPD, valPET, valphi, valPIXCON, 
 valPIXGDP, valPIXGDP_W, valPIXGVT, valPIXINV, valPK, valPL,  valPMT, valPP, 
 valPT, valPVA, valPVAT, valPWMG,  valQ, valRC, 
 valSG, valSH, valsh0, valsh1, valSROW, valTDH, valTIC, valTICT,  valTIKT, 
 ...
 valPX_INDEX, valPM_INDEX, valTOT,
  valAEEI, valEE, valNE, valEH, valTFC_product, valTFC,
 valTES, valTES_coal, valTES_oil, valTES_gas, valTES_nuc, valTES_hydro, valTES_solar, valTES_wind, valTES_bio, valTES_waste, valTES_geo, valTES_other,
 valMarinebunker, valAviationbunker,
 valCO2I, valCO2NE, valCO2I2, valCO2NE2, valCO2I3, valTCO2I, valTCO2NE, valCO2H, valTCO2H, valTCO2, 
 valCO2I2_FUELCOMB, valTCO2H_FUELCOMB, valTCO2I_FUELCOMB, 
