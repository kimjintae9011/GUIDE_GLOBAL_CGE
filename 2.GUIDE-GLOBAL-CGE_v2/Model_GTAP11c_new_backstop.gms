...
* -------------------------------------------------------------------
SCALAR seed_val / 1E-5 /;

LOOP((j,z)$(XSTO(j,z) <= 1E-6 and not (BS(j) and Z_OTH(z))),
* 1. 생산량(XSTO)에 아주 작은 Seed 값 부여
    XSTO(j,z) = seed_val;
    
* 2. 중국(02_CHN)의 동일 산업(j) 투입 구조 비율을 계산하여 Seed 값에 분배
* (중국의 해당 산업 총비용 대비 각 요소의 비중을 차용)
    LDO(j,z)    = seed_val * (LDO(j,'02_CHN') / XSTO(j,'02_CHN'));
    TIWO(j,z)   = seed_val * (TIWO(j,'02_CHN') / XSTO(j,'02_CHN'));
    TIPO(j,z)   = seed_val * (TIPO(j,'02_CHN') / XSTO(j,'02_CHN'));
    
    LOOP(k, 
        RKDO(k,j,z) = seed_val * (RKDO(k,j,'02_CHN') / XSTO(j,'02_CHN'));
        TIKO(k,j,z) = seed_val * (TIKO(k,j,'02_CHN') / XSTO(j,'02_CHN'));
    );
    
    LOOP(i, 
        DIO(i,j,z)  = seed_val * (DIO(i,j,'02_CHN') / XSTO(j,'02_CHN'));
    );
    
* 3. 이제 투입 비용의 합이 XSTO와 완벽히 일치하므로 가격(PPO)은 1.0이 됨
    PPO(j,z) = 1.0;
    PVAO(j,z) = 1.0;
    VAO(j,z) = LDO(j,z) + SUM(k, RKDO(k,j,z));
);
