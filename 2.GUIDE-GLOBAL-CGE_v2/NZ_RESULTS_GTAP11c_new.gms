...
*================== DAC=== ===========================================================
 valQDAC(z,time,'NZ') = QDAC.l(z,time);
 valDAC_growth_rate(z,time,'NZ')  = DAC_growth_rate(z,time);
 valDAC_elec(z,time,'NZ') = QDAC.l(z,time)*DAC_Tech_Coeff('18_ELEC') ;
 valswitchDAC(z,time,'NZ') = switchDAC(z,time);
*=====================================================================================================

 Avg_Tax_Wedge(ene,z,time) =  SUM(j, (CTAX.l(z,time) * CO2FACTOR2(ene,j,z,time)) * DE.l(ene,j,z,time)) / SUM(j, DE.l(ene,j,z,time));
 P_EFF_NZ(ene,z,time)  = PC.l(ene,z,time) * (1 + Avg_Tax_Wedge(ene,z,time));
