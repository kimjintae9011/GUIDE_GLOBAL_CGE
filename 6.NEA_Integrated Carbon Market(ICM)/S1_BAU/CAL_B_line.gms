...
SCALAR Price_USD_MWh        USD per MWh / 107.14 /;
 ...
 DAC_MAX('17_PAO')         = sum(j,PERMITO(j,'17_PAO'))*0.6;
...
Transfer_Amount(BS, Conv, I_BS, Z_GRN) "Conv에서 BS로 넘겨줄 Beta 총량"
gr_BVAT(time) "신기술 생산성의 연도별 성장률"
Temp_Cap(BS, Z_GRN, time)
;
...
*=============================================================================
 ...
 B_VA_t('18_TnD',z,time)$[ord(time) gt 1]
                        = B_VA_t('18_TnD',z,time-1)*[1+SolarWind_TFP_BAU];
...
*==============================================================================
LOOP(Z_GRN,
* 1. 가격 신호(Signal) 계산 (기존과 동일)
    Signal(BS, Z_GRN, time)$[ord(time) gt 1] 
        = MAX( 0, ( SUM(Conv$map_Conv(BS, Conv), PP.L(Conv, Z_GRN, time-1)) / PP.L(BS, Z_GRN, time-1) ) - 1 );
        
* 2. [핵심] exp() 없는 가중치(Multiplier) 계산
* PTSF가 0.1 이하이면 무조건 0.95
* PTSF가 0.15 이상이면 무조건 1.05
* 0.1 ~ 0.15 사이 구간에서는 0.95에서 1.05로 부드럽게 이어지는 직선 비율 적용
...
*==============================================================================
 ...
 WC.LO(j,z,time)$(ord(time) gt 1)       = 0.0001*WC.l(j,z,time-1);
 phi.LO(z,time)$(ord(time) gt 1)           = 0.0001*phi.l(z,time-1);  
...
execute_unload 'Input_CGE/B_line.gdx',
