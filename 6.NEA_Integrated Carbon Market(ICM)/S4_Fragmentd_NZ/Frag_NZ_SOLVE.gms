...
*==============================================================================
 ...
 CTAX.fx('04_RUS',time) = CTAXO('04_RUS') ;
 CTAX.fx('05_MNG',time) = CTAXO('05_MNG') ;
 CTAX.fx('06_PRK',time) = CTAXO('06_PRK') ;
...
*==============================================================================                     
 ...
 KD.fx(k,j,z,time)${[ord(time) gt 1] and KDO(k,j,z)}
                       = KD.l(k,j,z,time-1) * [1 - delta_t(j,z,time-1)] + IND.l(k,j,z,time-1);
...
*============================================================================== 
 PERMIT_TOTAL.fx(PERMIT_Z,time)$[ord(time) gt 1]
                       = PERMIT_TOTALO(PERMIT_Z)*S3_PERMIT(PERMIT_Z,time);  
...
LOOP(z,
    IF(CTAX.L(z, time) >= DAC_TRIGGER,
        ...
        ELSE
            DAC_growth_rate(z, time+1) = DAC_growth_rate(z, time) *1.15;
        );
...
*==============================================================================
* [공정 혁신] 정유/코크스 산업(PETROLCOAL)의 석탄 투입 감축 및 전력화(Electrification)
*==============================================================================
    IF(time.val <= 2019,
        phi_trans(time) = 0.0;
